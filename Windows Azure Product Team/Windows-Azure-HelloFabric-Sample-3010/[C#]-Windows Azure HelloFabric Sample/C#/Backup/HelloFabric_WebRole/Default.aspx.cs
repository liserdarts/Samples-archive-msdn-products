...
namespace Microsoft.Samples.ServiceHosting.HelloFabric
{
    ...
    {
        private const string NewsServer = "news.microsoft.com";

        protected void Page_Load(object sender, EventArgs e)
        {           
            // Check if we are running in the fabric
            if (RoleEnvironment.IsAvailable)
            {
                Label3.Text = "Running in the fabric." ;
                Label1.Text = RoleEnvironment.GetConfigurationSettingValue("BannerText");
                if (Request.IsSecureConnection)
                {
                    Label2.Text = "Running over a secure connection.";
                }
                else
                {
                    Label2.Text = "Not running over a secure connection. Please see the documentation to test this sample with SSL.";
                }
            }
            else
            {
                Label3.Text = "Running outside the fabric.";
                Label2.Text = "";
        ...
        private void getCurrentTime()
        {
            ...
            try
            {
                endpoint = RoleEnvironment.Roles["HelloFabric_WorkerRole"].Instances[0].InstanceEndpoints["DayTime"].IPEndpoint;
            }
            catch (Exception)
            {
        ...
        {
            // This code crashes by design to show the logging feature if a role throws unhandled exceptions
            string nullString = null;
            int dummy = nullString.Length;
        }
        ...
        protected void NntpButton_Click(object sender, EventArgs e)
        {
            NntpResponsePanel.Visible = false;
            if (CanResolveAddress(NewsServer))
            {
                using (TcpClient client = new System.Net.Sockets.TcpClient(NewsServer, 119))
                {
                    using (NetworkStream netStream = client.GetStream())
                    {
                        using (StreamWriter writer = new StreamWriter(netStream))
                        {
                            using (StreamReader reader = new StreamReader(netStream))
                            {
                                ...
                                writer.WriteLine(NntpCommand.Text);
                                writer.WriteLine("quit");
                                writer.Flush();

                                NntpResponsePanel.Visible = true;
                                NntpResponseLabel.Text = reader.ReadToEnd();
                            }
                        }
                    }
                }
