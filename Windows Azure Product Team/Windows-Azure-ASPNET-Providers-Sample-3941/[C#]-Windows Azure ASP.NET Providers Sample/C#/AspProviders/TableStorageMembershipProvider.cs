...
namespace Microsoft.Samples.ServiceHosting.AspProviders
{
    ...
    [CLSCompliant(false)]
    public sealed class MembershipRow : TableServiceEntity, IComparable
    {
    ...
    public class TableStorageMembershipProvider : MembershipProvider
    {
        ...
        /// </summary>
        /// <returns>Returns the number of users within the online time window.</returns>
        ...
        private string EncodePassword(string pass, int passwordFormat, string salt)
        {
            ...
            { // MembershipPasswordFormat.Hashed
                using (HashAlgorithm s = HashAlgorithm.Create(Membership.HashAlgorithmType))
                {
                    if (s is KeyedHashAlgorithm)
                    {
                        KeyedHashAlgorithm keyed = s as KeyedHashAlgorithm;
                        byte[] bKey = new byte[keyed.Key.Length];
                        for (int offset = 0, copied = 0; offset < bKey.Length; offset += copied)
                        {
                            copied = Math.Min(bSalt.Length, bKey.Length - offset);
                            Buffer.BlockCopy(bSalt, 0, bKey, offset, copied);
                        }
                        keyed.Key = bKey;
                        bRet = keyed.ComputeHash(bIn);
                    }
                    else
                    {
