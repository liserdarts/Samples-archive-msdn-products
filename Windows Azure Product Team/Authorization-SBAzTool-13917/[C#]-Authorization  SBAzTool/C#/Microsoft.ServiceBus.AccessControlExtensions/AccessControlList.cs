                ...
                (new SegmentRuleGroup
                     {
                         ...
                         RuleGroup =
                             (from g in this.serviceClient.RuleGroups.Expand("RelyingPartyRuleGroups") 
                              where g.Name == prefix.Uri.AbsoluteUri || g.Name == DefaultRuleGroupName select g).
                             FirstOrDefault()
                     });
                    ...
                    (new SegmentRuleGroup
                         {
                             ...
                             SegmentName = segment,
                             RuleGroup =
                                 (from g in this.serviceClient.RuleGroups.Expand("RelyingPartyRuleGroups") where g.Name == prefix.Uri.AbsoluteUri select g).
                                 FirstOrDefault()
                         });
            }
        ...
        public AccessControlRule AddRule(IdentityReference identity, Right right)
        {
            var rule = (from r in this.localRules
                        where
                            ...
                            r.Condition.IssuerName == identity.IssuerName && r.Right.ClaimType == right.ClaimType &&
                            r.Right.ClaimValue == right.ClaimValue
