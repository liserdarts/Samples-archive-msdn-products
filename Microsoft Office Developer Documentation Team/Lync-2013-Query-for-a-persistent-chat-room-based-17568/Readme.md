...
<p><span style="font-size:small">The PersistentChat_RoomQuery Lync SDK sample application displays a form containing a text box and list box. The user enters a search string in the text box, and then chooses
<strong>Go</strong>. The application uses the <strong>State</strong> property of the
<strong>RoomManager</strong> class to determine whether the room manager is reachable. The application uses the
<strong>BeginQueryRooms()</strong> method of the <strong>RoomManager</strong> class to search for room title matches. All instances of the
<strong>Room</strong> class whose title matches the user&rsquo;s query string are added to a
<strong>Dictionary</strong> object. To display results, the application lists room titles from the
<strong>Dictionary</strong> object to a list box on the form.</span></p>
