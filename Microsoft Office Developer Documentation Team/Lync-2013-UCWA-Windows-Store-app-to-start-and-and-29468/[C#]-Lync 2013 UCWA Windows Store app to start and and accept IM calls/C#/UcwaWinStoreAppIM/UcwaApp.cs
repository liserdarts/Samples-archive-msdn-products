...
namespace WinStoreUcwaAppIM
{
    public class UcwaApp 
    {
        ...
        public UcwaAppMessagingInviteReceivedEventHandler OnMessagingInviteReceived; 
        ...
        public UcwaAppOnlineMeetings OnlineMeetings { get; private set; }
        public UcwaAppCommunication Communication { get; private set; }
        public UcwaAppPeople People { get; private set; }
        public UcwaAppBatchRequests Batch { get; private set; }
        public UcwaAppPolicies Policies { get; private set; }
        ...
        public async Task<HttpStatusCode> SignIn(string userName, string password, UcwaAppAuthenticationTypes authType=UcwaAppAuthenticationTypes.Password)
        {
            ...
            try
            {
                if (authType == UcwaAppAuthenticationTypes.Windows)
                {
                    this.Transport.SetAuthenticationType(authType);
                }
                ...
                timer = new Timer( async (e) =>
                    { await this.Transport.PostResourceAsync(
                        this.ApplicationResource.GetEmbeddedResource("me").GetLinkUri("reportMyActivity"), 
                        (HttpContent)null);
                    },
                    null,
                    new TimeSpan(0, 0, 0),
                ...
                this.OnlineMeetings = new UcwaAppOnlineMeetings(this);
                this.People = new UcwaAppPeople(this);
                this.Communication = new UcwaAppCommunication(this);
                this.Batch = new UcwaAppBatchRequests(this);
                this.Policies = new UcwaAppPolicies(this);
        ...
        {
            // need to handle the Windows authentication type here when the user name and password are not provided
            // to do
            // ...
            string domain = this.userName.Contains("@") ? this.userName.Split('@')[1] : null;

            if (domain == null || this.authenticationType == UcwaAppAuthenticationTypes.Windows)
            {
                Exception ex = new Exception("Windows authentication type not properly implemented.");
                if (OnErrorReported != null)
                    OnErrorReported(ex);
                return new UcwaAppOperationResult(HttpStatusCode.NotImplemented, ex);

                //if (UserInformation.NameAccessAllowed)
                //{
                //    var session = await UserInformation.GetSessionInitiationProtocolUriAsync();
                //    if (session != null)
                //    {
                //        domain = session.DnsSafeHost;  
                //    }
                //}
            }
            this.discoverFromInternalDomain = discoverFromInternalDomain;
        ...
        private OAuthToken GetOAuthToken(string responseData)
        {
            ...
            if (Windows.Data.Json.JsonObject.TryParse(responseData, out json))
                if (json.ContainsKey("access_token") && json.ContainsKey("token_type"))
                {
                    ...
                    var et = json.GetNamedValue("expires_in");
                    double etNumber = 0;
                    if (et != null)
                        etNumber = et.GetNumber();

                    if (at == null || tt == null)
                        return null;
                    oAuth20Token = new OAuthToken(tt.GetString(), at.GetString(), etNumber);
                    return oAuth20Token;
                }
            return oAuth20Token;
        }
