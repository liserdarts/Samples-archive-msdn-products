...
namespace WinStoreUcwaAppIM
{
    public class UcwaAppMe : UcwaAppEventHandlers
    {
        ...
        private async Task<UcwaAppOperationResult> GetPhotoStream(string uri=null)
        {
            if (string.IsNullOrEmpty(uri))
                uri = this.Resource.GetEmbeddedResourceUri("photo");
            if (string.IsNullOrEmpty(uri)) return null;
            var result = await Transport.GetPhotoStreamAsync(uri);
            this.Photo = result.ResponseBodyStream;
            return result;
        }
