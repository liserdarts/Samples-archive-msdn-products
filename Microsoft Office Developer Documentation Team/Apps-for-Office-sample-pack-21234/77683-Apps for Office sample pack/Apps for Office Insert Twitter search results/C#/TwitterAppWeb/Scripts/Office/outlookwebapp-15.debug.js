// Exchange Version 15.00 (Build 0463.000) 
// ---------------------------------------------------------------------
...
OSF.DDA.OutlookAppOm = function OSF_DDA_OutlookAppOm(officeAppContext, targetWindow, appReadyCallback) {
    ...
    if (appReadyCallback) {
        this._invokeHostMethod$i$0(1, 'GetInitialData', null, this.$$d__getInitialDataResponseHandler$p$0);
    }
}
...
OSF.DDA.OutlookAppOm.prototype = {
    ...
    getUserIdentityTokenAsync: function OSF_DDA_OutlookAppOm$getUserIdentityTokenAsync(callback, userContext) {
        ...
        this._invokeHostMethod$i$0(2, 'GetUserIdentityToken', null, function(resultCode, response) {
            if (resultCode) {
                OSF.DDA.OutlookAppOm._createAsyncResult$i(null, 1, 'Internal protocol error: ' + resultCode, userContext);
            }
            else {
    ...
    _dictionaryToDate$i$0: function OSF_DDA_OutlookAppOm$_dictionaryToDate$i$0(input) {
        ...
        retValue.setFullYear(input['year']);
        retValue.setMonth(input['month']);
        retValue.setDate(input['date']);
        retValue.setHours(input['hours']);
        retValue.setMinutes(input['minutes']);
        retValue.setSeconds(input['seconds']);
        retValue.setMilliseconds((!input['milliseconds']) ? 0 : input['milliseconds']);
        if (isNaN(retValue.getTime())) {
            throw Error.format('The input does not resolve to a valid date');
        }
        return retValue;
    },
...
$h.Entities.prototype = {
                ...
                if ($h.ScriptHelpers.dictionaryContainsKey(perEntityTypeDictionary, name)) {
                    switch (entityTypeKey) {
                        ...
                        case 'MeetingSuggestions':
                            this._filteredEntitiesCache$p$0[name] = $h.Entities._getExtractedObjects$i($h.MeetingSuggestion, perEntityTypeDictionary, name, this.$$d__createMeetingSuggestion$p$0);
                            break;
                        case 'Contacts':
    ...
    _getMeetingSuggestions$p$0: function $h_Entities$_getMeetingSuggestions$p$0() {
        ...
        if (!this._meetingSuggestions$p$0) {
            this._meetingSuggestions$p$0 = $h.Entities._getExtractedObjects$i($h.MeetingSuggestion, this._data$p$0, 'MeetingSuggestions', this.$$d__createMeetingSuggestion$p$0);
        }
...
$h.MeetingSuggestion.prototype = {
    ...
    _getStartTime$p$0: function $h_MeetingSuggestion$_getStartTime$p$0() {
        var time = this._data$p$0['StartTime'];
    ...
    _getEndTime$p$0: function $h_MeetingSuggestion$_getEndTime$p$0() {
        var time = this._data$p$0['EndTime'];
        var resolvedTime = $h.MeetingSuggestionTimeDecoder.resolve(time, this._dateTimeSent$p$0);
        if (resolvedTime.getTime() !== time.getTime()) {
            return OSF.DDA.OutlookAppOm._instance$p.convertToUtcClientTime(OSF.DDA.OutlookAppOm._instance$p._dateToDictionary$i$0(resolvedTime));
        }
...
$h.InitialData.prototype = {
    ...
    get__dateTimeSent$i$0: function $h_InitialData$get__dateTimeSent$i$0() {
        return this._data$p$0['dateTimeSent'];
    },
...
/* OsfWebAppCommon.js - this file is no longer needed for OSF. We need to have a OWA specific implmentation of save settings, and since
   OSF.DDA.Settings is defined with Object.defineProperties, we need to override the entire object.  Only "saveAsync" is OWA specific in the
   OSF.DDA.Settings definition.
   O15:2353091 tracks moving this to OWA script# code and working with OSF to reduce the duplicate code.
 */
/* Version: 15.0.4128.1005 */
/*
