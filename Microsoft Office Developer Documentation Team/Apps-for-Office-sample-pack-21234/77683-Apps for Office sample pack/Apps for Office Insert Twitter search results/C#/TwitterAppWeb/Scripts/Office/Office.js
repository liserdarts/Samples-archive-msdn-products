...
OSF.DDA.MethodDispId={
	...
	dispidGetWholeDocumentMethod: 77,
...
OSF.DDA.EventDispId={
	...
	var initOSFModule=function OSF$initOSFModule() {
		...
		OSF.DDA.AsyncMethodCall=function OSF_DDA_AsyncMethodCall(requiredParameters, supportedOptions, privateStateCallbacks, onSucceeded, onFailed, fixOptions, displayName) {
			...
			function OSF_DAA_AsyncMethodCall$VerifyArguments(params, args) {
				for(var name in params) {
					...
					var arg=args[name];
					if(param["enum"] && typeof arg !="string") {
			...
			this.getCallArgs=function(suppliedArgs) {
				return OSF_DAA_AsyncMethodCall$ExtractOptions(suppliedArgs);
			}
		}
		...
		OSF.DDA.AsyncMethodCalls=(function() {
			...
			define({
				...
				onSucceeded: function(dataDescriptor, caller, callArgs) {
					var data=dataDescriptor[Microsoft.Office.WebExtension.Parameters.Data];
					var ret;
					switch(callArgs[Microsoft.Office.WebExtension.Parameters.FileType]) {
						case Microsoft.Office.WebExtension.FileType.Base64:
							ret=OSF.OUtil.encodeBase64(data);
							break;
						case Microsoft.Office.WebExtension.FileType.Text:
						default:
							ret=data;
							break;
					}
		...
		OSF.DDA.Document=function OSF_DDA_Document(officeAppContext) {
			var mode;
		...
		OSF.DDA.JsomDocument=function OSF_DDA_JsomDocument(officeAppContext, bindingFacade, settings) {
			OSF.DDA.JsomDocument.uber.constructor.call(this, officeAppContext);
			...
			var initialize=function OSF__OfficeAppFactory$initialize() {
				...
				getAppContextAsync(_WebAppState.window, function (appContext) {
					...
					var postLoadLocaleStringInitialization=function OSF__OfficeAppFactory_initialize$postLoadLocaleStringInitialization() {
						...
						errorMgr.addErrorMessage(errorCodes.ooeDocumentIsTooLarge,  { name : stringNS.L_DataReadError , message : stringNS.L_DocumentIsTooLarge });
						...
						function initializeSettings() {
							...
							_settings=new OSF.DDA.Settings(OSF.DDA.SettingsManager.deserializeSettings(serializedSettings));
						}
