...
function transmitChunk() {
    ...
    // but the actual value returned is in Bytes (see the HTML for the drop-down in WD_WordDocumentEmitter_js.html)
    ...
    // In our case we choose Text, becuase we have also created a PowerPoint sample called PowerPointEBookPublisher
            ...
            for (slice = 0; slice < totalSlices; slice++) {
                ...
                var currentSlice = currentFile.getSliceAsync(slice, function (result) {
                    if (result.status == "succeeded") {
                        ...
                        rawData.setAttribute("style", "cursor:pointer");
                        rawData.setAttribute("onclick", "showChunk('" + sliceData + "');");
                        rawData.appendChild(rawDataDetails);
...
function showChunk(dataChunk) {
    ...
    dialogStyleSheet.href = '/content/jquery-ui.css';
    ...
    dialogScriptUI.src = '/scripts/jquery-ui.js';
