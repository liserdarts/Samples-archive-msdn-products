...
{
#ifndef DEBUG
	//just return true and do not log informational messages to the event log
	if (wType == EVENTLOG_INFORMATION_TYPE)
	{
		return true;
	}
#endif
								...
								LPCTSTR lpszEventMessageDll)
{
	...
	ZeroMemory(szKey, _MAX_PATH*2*sizeof(TCHAR));
	...
	if (lRet == ERROR_SUCCESS)
	{
		...
		ZeroMemory(szPathName, _MAX_PATH*2*sizeof(TCHAR));
...
void CXEventLog::SetAppName(LPCTSTR lpszApp)
{
	...
	if (m_pszAppName)
	{
		ZeroMemory(m_pszAppName, _MAX_PATH*2*sizeof(TCHAR));
