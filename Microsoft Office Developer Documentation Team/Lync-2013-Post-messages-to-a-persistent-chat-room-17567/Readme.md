...
<p><span style="font-size:small">The PersistentChat_PostMessage Lync SDK sample application features the
<strong>JoinedState</strong> property, the FollowedRooms collection, and the <strong>
BeginSendMessage()</strong> method. The application gets the list of followed rooms from the FollowedRooms collection of the
<strong>RoomManager</strong> class. The user selects a particular room from a drop-down list box, and enters a message in a text box. When the user clicks
<strong>Send</strong>, the application uses the <strong>JoinedState</strong> property to verify that the user is connected to the room, and then uses the
<strong>BeginSendMessage</strong> method of the <strong>Room</strong> class to send the message.</span></p>
