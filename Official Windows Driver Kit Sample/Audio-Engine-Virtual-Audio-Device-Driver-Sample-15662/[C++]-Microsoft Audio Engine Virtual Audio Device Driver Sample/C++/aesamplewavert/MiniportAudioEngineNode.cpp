...
STDMETHODIMP_(NTSTATUS) CMiniportWaveRT::GetAudioEngineDescriptor(_In_ ULONG _ulNodeId, _Out_ KSAUDIOENGINE_DESCRIPTOR *_pAudioEngineDescriptor)
{
    ...
    {
        _pAudioEngineDescriptor->nHostPinId = KSPIN_WAVE_RENDER_SINK_SYSTEM;
        _pAudioEngineDescriptor->nOffloadPinId = KSPIN_WAVE_RENDER_SINK_OFFLOAD;
        _pAudioEngineDescriptor->nLoopbackPinId = KSPIN_WAVE_RENDER_SINK_LOOPBACK;
...
)
{
    ...
    switch (_formatType)
    {
        ...
        case eSupportedDeviceFormats:
            ...
            *_pulFormatSize = sizeof(KSMULTIPLE_ITEM) + sizeof(KSDATAFORMAT_WAVEFORMATEXTENSIBLE) * SIZEOF_ARRAY(AudioEngineSupportedDeviceFormats);
            break;
        default:
...
STDMETHODIMP_(NTSTATUS) CMiniportWaveRT::GetSupportedDeviceFormats(_In_  ULONG _ulNodeId, _Out_ KSMULTIPLE_ITEM* _pFormat, _In_ ULONG _ulBufferSize)
{
    ...
    pKsMulti->Size = sizeof(KSMULTIPLE_ITEM) + sizeof(KSDATAFORMAT_WAVEFORMATEXTENSIBLE) * SIZEOF_ARRAY(AudioEngineSupportedDeviceFormats);
    pKsMulti->Count = 2;

    IF_TRUE_ACTION_JUMP(_ulBufferSize < pKsMulti->Size, ntStatus = STATUS_BUFFER_TOO_SMALL, Exit);


    RtlCopyMemory((PVOID)(pKsMulti + 1), (PVOID)AudioEngineSupportedDeviceFormats,
        sizeof(KSDATAFORMAT_WAVEFORMATEXTENSIBLE) * SIZEOF_ARRAY(AudioEngineSupportedDeviceFormats));
...
STDMETHODIMP_(NTSTATUS) CMiniportWaveRT::SetDeviceChannelVolume(_In_  ULONG _ulNodeId, _In_ UINT32 _uiChannel, _In_  LONG  _Volume)
{
    ...
    LONG lVolume = _Volume < MIN_VOLUME_STEPPINGS ? MIN_VOLUME_STEPPINGS :
                   _Volume > MAX_VOLUME_STEPPINGS ? MAX_VOLUME_STEPPINGS : (_Volume / VOLUME_STEPPING_DELTA);
...
NTSTATUS CMiniportWaveRT::GetVolumeChannelCount(_Out_  UINT32 *_pulChannelCount)
{
    ...
    *_pulChannelCount = DEVICE_MAX_CHANNELS;
     return ntStatus;
}
#pragma code_seg("PAGE")
NTSTATUS CMiniportWaveRT::GetVolumeSteppings(_Out_writes_bytes_(_ui32DataSize) PKSPROPERTY_STEPPING_LONG _pKsPropStepLong, _In_  UINT32 _ui32DataSize)
{
    ...
    ASSERT(ulChannelCount <= DEVICE_MAX_CHANNELS);
    if (ulChannelCount > DEVICE_MAX_CHANNELS)
    {
...
NTSTATUS CMiniportWaveRT::GetChannelVolume(_In_  UINT32 _uiChannel, _Out_ LONG *_pVolume)
{
    ...
    if (_uiChannel == 0xffffffff)
    {
        *_pVolume = m_plVolumeLevel[0];
    }
    else
    {
        ASSERT(_uiChannel <= DEVICE_MAX_CHANNELS);
...
NTSTATUS CMiniportWaveRT::GetChannelPeakMeter(_In_  UINT32 _uiChannel, _Out_  LONG *_plPeakMeter)
{
    ...
    if (_uiChannel >= DEVICE_MAX_CHANNELS)
    {
        return STATUS_INVALID_PARAMETER;
    }
