...
PVOID
DsmpAllocatePool(
    _In_ __drv_strictTypeMatch(__drv_typeExpr) IN POOL_TYPE PoolType,
...
--*/
{
    ...
    *BytesAllocated = 0;
    ...
    if (NT_SUCCESS(status)) {
        ...
        #pragma warning(suppress: 6014) // Block isn't leaked, this function is marked as an allocator
...
DsmpParseDeviceID(
    ...
    _When_(DeviceIdType == DSM_DEVID_SERIAL_NUMBER, _Out_opt_) OUT PSTORAGE_IDENTIFIER_CODE_SET CodeSet
    )
/*++
...
--*/
{
    ...
    if (vendorIDLength > 0) {
        ...
        if (!vendorID) {
            ...
            TracePrint((TRACE_LEVEL_ERROR,
                        TRACE_FLAG_PNP,
                        "DsmpBuildDeviceNameLegacy (DevInfo %p): Failed to allocate memory for vendor ID.\n",
                        DeviceInfo));
    ...
    if (productIDLength > 0) {
        ...
        if (!productID) {
            ...
            TracePrint((TRACE_LEVEL_ERROR,
                        TRACE_FLAG_PNP,
                        "DsmpBuildDeviceNameLegacy (DevInfo %p): Failed to allocate memory for product ID.\n",
                        DeviceInfo));
    ...
    if (serialIDLength > 0) {
        ...
        if (!serialID) {
            ...
            TracePrint((TRACE_LEVEL_ERROR,
                        TRACE_FLAG_PNP,
                        "DsmpBuildDeviceNameLegacy (DevInfo %p): Failed to allocate memory for serial ID.\n",
                        DeviceInfo));
    ...
    if (vendorIDLength) {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            TracePrint((TRACE_LEVEL_ERROR,
                        TRACE_FLAG_PNP,
                        "DsmpBuildDeviceNameLegacy (DevInfo %p): Failed to concatenate vendor ID to device name.\n",
                        DeviceInfo));
    ...
    if (productIDLength) {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            TracePrint((TRACE_LEVEL_ERROR,
                        TRACE_FLAG_PNP,
                        "DsmpBuildDeviceNameLegacy (DevInfo %p): Failed to concatenate product ID to device name.\n",
                        DeviceInfo));
    ...
    if (serialIDLength) {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            TracePrint((TRACE_LEVEL_ERROR,
                        TRACE_FLAG_PNP,
                        "DsmpBuildDeviceNameLegacy (DevInfo %p): Failed to concatenate serial number to device name.\n",
                        DeviceInfo));
    ...
    )
{
    ...
    TracePrint((TRACE_LEVEL_INFORMATION,
                TRACE_FLAG_GENERAL,
                "DsmpReportTargetPortGroupsSyncCompletion: DeviceObject %p, IRP %p, Context %p\n",
...
__Retry_DsmpReportTargetPortGroups:
    ...
    //
    // Handle the case where we get back STATUS_BUFFER_OVERFLOW b/c the input
    // buffer was larger than necessary.
    //
    if (status == STATUS_BUFFER_OVERFLOW &&
        SrbGetDataTransferLength(srb) < targetPortGroupsInfoLength) {
        status = STATUS_SUCCESS;
    }
...
__Exit_DsmpReportTargetPortGroupsAsync:
    ...
    if (status != STATUS_PENDING) {
        ...
        if (tpgCompletionContext->Irp != NULL) {
        ...
        //
        // This indicates we will let Completion Routine do the clean up.
        //
        tpgCompletionContext->Irp = NULL;
    }
