...
--*/
{
    ...
    if (DsmpIsSymmetricAccess(FailingDevice)) {
        ...
        DsmpSetLBForPathRemoval(DsmContext, FailingDevice, NULL);

    } else {

        DsmpSetLBForPathRemovalALUA(DsmContext, FailingDevice, NULL);
    }
...

{
    ...
    if (DsmpIsSymmetricAccess(pDevInfo)) {
        ...
        DsmpSetLBForPathRemoval(DsmContext, pDevInfo, Group);

    } else {

        DsmpSetLBForPathRemovalALUA(DsmContext, pDevInfo, Group);
    }
    ...
    switch (groupEntry->LoadBalanceType) {
        ...
        case DSM_LB_LEAST_BLOCKS: {
            ...
            if (groupEntry->UseCacheForLeastBlocks) {
                ...
                //
                // Make sure the SRB and CDB are available before performing the
                // load balance policy.
                //
                if (Srb) {
                    cdb = SrbGetCdb(Srb);
                    if (cdb) {     
                        ...
                        //
                        // CDB is NULL so just use the same path.
                        //
                        forceUseSamePath = TRUE;
                    }
                } else {
                    //
                    // SRB is NULL so just use the same path.
                    //
                    forceUseSamePath = TRUE;
                }
            } 
...

{
    ...
    if (failGroup) {
        ...
        if (DsmIsReadWrite(opCode)) {
            ...
            InterlockedExchangeAdd64((LONGLONG volatile*)&(failGroup->OutstandingBytesOfIO), -(LONGLONG)dataTransferLength);
        }
...
--*/
{
    ...
    //
    status = DsmpSetLBForPathRemoval(DsmContext, FailingDeviceInfo, NULL);
...
__Exit_After_Releasing_Resources_DsmpPhase1ProcessPathFailingALUA:
    ...
    if (context->Irp != NULL && !NT_SUCCESS(Irp->IoStatus.Status)) {

        //
        // We are called synchronously from the caller's context (thread).
        //
...
--*/
{
    ...
    if ((scsiStatus == SCSISTAT_GOOD) &&
        ...
        TracePrint((TRACE_LEVEL_INFORMATION,
                    TRACE_FLAG_RW,
                    "DsmpPhase2ProcessPathFailingALUA (DevObj %p): RTPG using %p (path %p) succeeded.\n",
                    DeviceObject,
                    deviceInfo,
                    deviceInfo->FailGroup->PathId));
