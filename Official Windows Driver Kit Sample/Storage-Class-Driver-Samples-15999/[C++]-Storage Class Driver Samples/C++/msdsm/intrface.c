    ...
    *NewPathId = NULL;
    ...
    if (failGroup->Count == 0) {
        ...
        for (entry = failGroup->ZombieGroupList.Flink; entry != &(failGroup->ZombieGroupList); entry = entry->Flink) {
            ...
            pathId = DsmpSetNewPathUsingGroup((PDSM_CONTEXT)DsmContext, group->Group);
...

{
    ...
    irql = ExAcquireSpinLockShared(&(dsmContext->DsmContextLock));
            ...
            if (completionContext && completionContext->DeviceInfo) {
                ...
                if (oldPath) {
                    ...
                    if (InterlockedCompareExchange((LONG volatile*)&oldPath->NumberOfRequestsInFlight, 0, 0) > 0) {
                        ...
                        InterlockedExchangeAdd64((LONGLONG volatile*)&(oldPath->OutstandingBytesOfIO), -(LONGLONG)(dataTransferLength));
                    }
                    ...
                    if (InterlockedCompareExchange((LONG volatile*)&oldPath->NumberOfRequestsInFlight, 0, 0) == 0) {
                        ...
                        if (oldPath->State == DSM_FG_PENDING_REMOVE) {
                            ...
                            //
                            DsmpConvertSharedSpinLockToExclusive(&(dsmContext->DsmContextLock));
                    ...
                    InterlockedExchangeAdd64((LONGLONG volatile*)&(failGroup->OutstandingBytesOfIO), dataTransferLength);
    ...
    if (lockInExclusiveMode) {
        ExReleaseSpinLockExclusive(&(dsmContext->DsmContextLock), irql);
    } else {
        ExReleaseSpinLockShared(&(dsmContext->DsmContextLock), irql);
    }
                    ...
                    if (senseKey == SCSI_SENSE_UNIT_ATTENTION) {
                        ...
                        if (addSenseCode == SCSI_ADSENSE_LUN_NOT_READY) {
                            ...
                            if (addSenseCodeQualifier == SPC3_SCSI_SENSEQ_ASYMMETRIC_ACCESS_STATE_TRANSITION ||
                                addSenseCodeQualifier == SPC3_SCSI_SENSEQ_TARGET_PORT_IN_STANDBY_STATE ||
                                addSenseCodeQualifier == SPC3_SCSI_SENSEQ_TARGET_PORT_IN_UNAVAILABLE_STATE) {
