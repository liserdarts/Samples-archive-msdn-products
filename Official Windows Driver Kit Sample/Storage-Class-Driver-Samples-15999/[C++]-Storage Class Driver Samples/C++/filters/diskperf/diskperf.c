...

{
    ...
    DebugPrint((2, "DiskPerfWmi: DeviceObject 0x%p Irp 0x%p\n",
                    DeviceObject, Irp));
    wmilibContext = &deviceExtension->WmilibContext;
    ...
    if ( !NT_SUCCESS(status) ) {
        DebugPrint((3, "DiskPerfWMI: IoAcquireRemoveLock failed: DeviceObject %p WMI Irp %p Status: %!STATUS!.\n",
...
//      case IrpNotWmi:
    ...
    if (lockHeld) {
        ...
        DebugPrint((2, "DiskPerfWmi: Releasing Lock: DeviceObject 0x%p Irp 0x%p\n",
        DeviceObject, Irp ));
        //
        // Release the remove lock
        //
