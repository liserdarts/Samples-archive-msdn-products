...
--*/
  ...
  //1.3 Online / Exit DEVSLP for the port if necessary
    PortPhyOnline(ChannelExtension);
...
--*/
    ...
    if (ChannelExtension->StateFlags.PoFxEnabled == 1) {
        if ( IsPortD3ColdEnabled(ChannelExtension) ||
             IsAhciDEVSLPEnabled(ChannelExtension) ||
             IsAcpiDEVSLPSupported(ChannelExtension) ) {
            // the link will be inactive, ignore the hot plug noise.
            ChannelExtension->StateFlags.IgnoreHotplugInterrupt = TRUE;
        }

        if (IsPortSoftwareManagedDEVSLPEnabled(ChannelExtension)) {
            PortPhySleep(ChannelExtension);
        }
    }
...
VOID
ReadIdentifyDeviceDataLogSATAPageCompletion (
    _In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
    _In_ PSCSI_REQUEST_BLOCK Srb
    )
{
    BOOLEAN shouldCompleteRequest = TRUE;
    ...
    if (Srb->SrbStatus == SRB_STATUS_SUCCESS) {
        ...
        PDEVSLP_TIMING_VARIABLES devSlpTiming = (PDEVSLP_TIMING_VARIABLES)((PUCHAR)ChannelExtension->DeviceExtension->ReadLogExtPageData + 0x30);

        if ( (pageHeader->RevisionNumber == IDE_GP_LOG_VERSION) && 
             (pageHeader->PageNumber == IDE_GP_LOG_IDENTIFY_DEVICE_DATA_SUPPORTED_PAGES) ) {

            StorPortCopyMemory(&ChannelExtension->DeviceExtension->DeviceSleepTiming, devSlpTiming, sizeof(DEVSLP_TIMING_VARIABLES));
        }
...
VOID
ReadIdentifyDeviceDataLogSupportedPagesCompletion (
    _In_ PAHCI_CHANNEL_EXTENSION ChannelExtension,
    _In_ PSCSI_REQUEST_BLOCK Srb
    )
{
    ...
    if (Srb->SrbStatus == SRB_STATUS_SUCCESS) {
        ...
        if ( (pageHeader->RevisionNumber == IDE_GP_LOG_VERSION) && 
             (pageHeader->PageNumber == IDE_GP_LOG_IDENTIFY_DEVICE_DATA_SUPPORTED_PAGES) &&
             (pageCount >= (IDE_GP_LOG_IDENTIFY_DEVICE_DATA_SATA_PAGE + 1)) &&
             (*(pageSupported + sizeof(IDENTIFY_DEVICE_DATA_LOG_PAGE_HEADER) + 1 + IDE_GP_LOG_IDENTIFY_DEVICE_DATA_SATA_PAGE) == 1) ) {

            // SATA page of Identify Device Log is supported, read this page
            ChannelExtension->DeviceExtension->SupportedGPLPages.IdentifyDeviceDataLogSATAPage = TRUE;
    ...
    )
{
    ...
    if ( (Srb->SrbStatus == SRB_STATUS_SUCCESS) && 
        ...
        // per ACS spec: The value of the General Purpose Logging Version word shall be 0001h
        ChannelExtension->DeviceExtension->SupportedGPLPages.NcqCommandErrorLog = (ChannelExtension->DeviceExtension->ReadLogExtPageData[IDE_GP_LOG_NCQ_COMMAND_ERROR_ADDRESS] > 0);
        ChannelExtension->DeviceExtension->SupportedGPLPages.CurrentDeviceInternalStatusDatalog = (ChannelExtension->DeviceExtension->ReadLogExtPageData[IDE_GP_LOG_CURRENT_DEVICE_INTERNAL_STATUS] > 0);
        ChannelExtension->DeviceExtension->SupportedGPLPages.SavedDeviceInternalStatusDatalog = (ChannelExtension->DeviceExtension->ReadLogExtPageData[IDE_GP_LOG_SAVED_DEVICE_INTERNAL_STATUS] > 0);

        if ( (ChannelExtension->DeviceExtension->ReadLogExtPageData[IDE_GP_LOG_IDENTIFY_DEVICE_DATA_ADDRESS] > 0) &&
             (IsAhciDEVSLPSupported(ChannelExtension)) ) {
            // look into Identify Device Log, read List of Supported Pages
            IssueReadLogExtCommand( ChannelExtension, 
                                    Srb, 
                                    IDE_GP_LOG_IDENTIFY_DEVICE_DATA_ADDRESS, 
                                    IDE_GP_LOG_IDENTIFY_DEVICE_DATA_SUPPORTED_PAGES,
                                    1,
                                    0,      // feature field
                                    &ChannelExtension->DeviceExtension->ReadLogExtPageDataPhysicalAddress, 
                                    (PVOID)ChannelExtension->DeviceExtension->ReadLogExtPageData, 
    ...
    )
{
  ...
  //1 update preserved commands per device needs.
    if (IsAtaDevice(&ChannelExtension->DeviceExtension->DeviceParameters)) {
      ...
      //1.3 DEVSLP Initialization
        if (IsAhciDEVSLPSupported(ChannelExtension)) {
            // Stop the port to set some values into Px->DEVSLP
            if ( P_NotRunning(ChannelExtension, ChannelExtension->Px) ) {
                // Init DETO and MDAT fields. 
                AHCI_DEVICE_SLEEP devslp;

                devslp.AsUlong = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension, &ChannelExtension->Px->DEVSLP.AsUlong);
                devslp.DETO = DeviceDevSlpDETO(ChannelExtension);
                devslp.MDAT = DeviceDevSlpMDAT(ChannelExtension);
                StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension, &ChannelExtension->Px->DEVSLP.AsUlong, devslp.AsUlong);
            }

            // Restart the port
            P_Running_StartAttempt(ChannelExtension, FALSE);

            // Enable DEVSLP if it's supported but not enabled
            if (ChannelExtension->DeviceExtension[0].IdentifyDeviceData->SerialAtaFeaturesEnabled.DEVSLP == 0) {
                UpdateSetFeatureCommands(ChannelExtension, IDE_FEATURE_INVALID, IDE_FEATURE_ENABLE_SATA_FEATURE, 0, IDE_SATA_FEATURE_DEVSLP);
            }
        }
    ...
    )
{
    ...
    if ( ((argument->Argument & ACPI_METHOD_DSM_LINKPOWER_FUNCTION_QUERY) != 0) && 
        ...
        argument = ACPI_METHOD_NEXT_ARGUMENT(argument);
        ACPI_METHOD_SET_ARGUMENT_INTEGER(argument,
                                         ACPI_METHOD_DSM_LINKPOWER_FUNCTION_QUERY);

        // argument 3 - Function-dependent package. not used for ACPI_METHOD_DSM_LINKPOWER_FUNCTION_QUERY
        argument = ACPI_METHOD_NEXT_ARGUMENT(argument);
        ACPI_METHOD_SET_ARGUMENT_INTEGER(argument, 0);
        argument->Type = ACPI_METHOD_ARGUMENT_PACKAGE;
        ...
        argument = &outputData->Argument[0];
        for (i = 0; i < outputData->Count; i++) {
            // StorAHCI doesn't support Port Multiplier. The Address should have 0xFFFF in lower two bytes.
            if ((argument->Argument & 0xFFFF) == 0xFFFF) {
                // Port number is in the higher 16 bits.
                ULONG portNumber = argument->Argument >> 16;
                if (IsPortValid(AdapterExtension, portNumber)) {
                    // Mark the port supports _DSM
                    AdapterExtension->PortExtension[portNumber]->StateFlags.SupportsAcpiDSM = TRUE;
                }
            }
            argument = ACPI_METHOD_NEXT_ARGUMENT(argument);
        }
  ...
  )
{
    if (IsAhciDEVSLPEnabled(ChannelExtension) && !IsAhciAggressiveDEVSLPSupported(ChannelExtension)) {
        ...
        if ( (ci == 0) && (sact == 0) && (ChannelExtension->StartState.ChannelNextStartState == StartComplete) ) {
            AHCI_SERIAL_ATA_STATUS  ssts;
            AHCI_COMMAND            cmd;

            ssts.AsUlong = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension, &ChannelExtension->Px->SSTS.AsUlong);

            // check the Link Power State, return if it's already in DEVSLP state.
            if (ssts.IPM == 8) {
                return;
            }
            
            // enter DEVSLP from slumber state
            if (ChannelExtension->AdapterExtension->CAP2.DESO == 1) {

                cmd.AsUlong = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension, &ChannelExtension->Px->CMD.AsUlong);

                // link should be in idle state.
                if ( (ssts.IPM != 6) && (cmd.ICC == 0) ) {
                    // in partial state, bring it to active
                    if (ssts.IPM == 2) {
                        // set into Active state.
                        cmd.ICC = 1;
                        StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension, &ChannelExtension->Px->CMD.AsUlong, cmd.AsUlong);
                    ...
                    ssts.AsUlong = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension, &ChannelExtension->Px->SSTS.AsUlong);
                    if (ssts.IPM != 6) {
                        StorPortStallExecution(20);  //20 microseconds
                    }
                    // don't bother to check link state again. if it cannot get into Slumber, the next step of putting into DEVSLP will be ignored by adapter
                }
            }

            cmd.AsUlong = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension, &ChannelExtension->Px->CMD.AsUlong);

            // Change LPM State. try to bring it into DEVSLP state.
            cmd.ICC = 8;
            StorPortWriteRegisterUlong(ChannelExtension->AdapterExtension, &ChannelExtension->Px->CMD.AsUlong, cmd.AsUlong);
        }
    }
...
{
    if (ChannelExtension->StateFlags.PoFxEnabled == 0) {
        return;
    }

    NT_ASSERT(IsPortSoftwareManagedDEVSLPEnabled(ChannelExtension));

    if (IsAcpiDEVSLPSupported(ChannelExtension)) {
        // ACPI method is implemented
        AhciPortAcpiDSMControl(ChannelExtension->AdapterExtension, ChannelExtension->PortNumber, TRUE);
    } else {
        // Adapter Native method
        AhciPortAhciPhySleep(ChannelExtension);
    }
    ...
    )
{
    ...
    if (IsPortSoftwareManagedDEVSLPEnabled(ChannelExtension)) {
        if (IsAcpiDEVSLPSupported(ChannelExtension)) {
            // ACPI method is implemented, invoke ACPI method to online the port
            AhciPortAcpiDSMControl(ChannelExtension->AdapterExtension, ChannelExtension->PortNumber, FALSE);
        } else {
            // DEVSLP is supported by adapter, de-assert DEVSLP.
            AHCI_SERIAL_ATA_STATUS ssts;
            ssts.AsUlong = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension, &ChannelExtension->Px->SSTS.AsUlong);

            if (ssts.DET == 8) {
                // only do this when the Link is in DEVSLP mode (value 8)
                AHCI_COMMAND cmd;
                cmd.AsUlong = StorPortReadRegisterUlong(ChannelExtension->AdapterExtension, &ChannelExtension->Px->CMD.AsUlong);
                ...
                // wait for suggested time to make sure everything is ready.
                StorPortStallExecution(DeviceDevSlpMDAT(ChannelExtension) + DeviceDevSlpDETO(ChannelExtension) + 1);  //20 microseconds
            }
        }
    }
