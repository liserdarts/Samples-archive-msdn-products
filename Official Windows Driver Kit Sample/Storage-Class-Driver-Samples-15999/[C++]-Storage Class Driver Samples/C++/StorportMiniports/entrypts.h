...
typedef struct _CHANNEL_STATE_FLAGS {
    ...
    ULONG PoFxEnabled : 1;
    ULONG PoFxActive : 1;
    ULONG SupportsAcpiDSM : 1;      // indicates if the system has _DSM method implemented to control port/device power
...
typedef struct _ATA_SUPPORTED_GPL_PAGES {
    BOOLEAN NcqCommandErrorLog;                 // Log Address 0x10
    BOOLEAN IdentifyDeviceDataLogSATAPage;      // Log Address 0x30, page 0x08. Only being retrieved if DEVSLP is supported
    BOOLEAN CurrentDeviceInternalStatusDatalog; // Log Address 0x24
    BOOLEAN SavedDeviceInternalStatusDatalog;   // Log Address 0x25
} ATA_SUPPORTED_GPL_PAGES, *PATA_IO_ATA_SUPPORTED_GPL_PAGES;
...
typedef struct _AHCI_DEVICE_EXTENSION {
    ...
    DEVSLP_TIMING_VARIABLES DeviceSleepTiming;
