...
--*/
{
  ...
  //3.4 Initializing the rest of PORT_CONFIGURATION_INFORMATION
    ...
    ConfigInfo->FeatureSupport |= STOR_ADAPTER_FEATURE_IO_LATENCY_CAP;
...
--*/
{
    ...
    // 2. If a port is in DEVSLP, de-assert DEVSLP
    if (AdapterExtension->StateFlags.PoFxEnabled == 1) {
        ULONG i;
        for (i = 0; i <= AdapterExtension->HighestPort; i++) {
            if ((AdapterExtension->PortExtension[i] != NULL) && IsAhciDEVSLPEnabled(AdapterExtension->PortExtension[i])) {
                // only do this if DEVSLP is supported natively by adaper and enabled.
                PortPhyOnline(AdapterExtension->PortExtension[i]);
            }
        }
    }
          ...
          // able to enter Partial state
            if (channelExtension->Px->CMD.ASP == 0) {
                ...
                if ((channelExtension->AutoPartialToSlumberInterval > 0) && (channelExtension->StartState.ChannelNextStartState == StartComplete) ) {
                    ...
                    StorPortRequestTimer(AdapterExtension, channelExtension->WorkerTimer, AhciAutoPartialToSlumber, channelExtension, channelExtension->AutoPartialToSlumberInterval * 1000, 20000);
                }
    ...
    switch (ControlType) {
        ...
        case ScsiUnitPoFxPowerInfo: {
            ...
            if (IsPortValid(adapterExtension, storAddrBtl8->Path)) {
                ...
                BOOLEAN                 reportF1State =  IsPortSoftwareManagedDEVSLPEnabled(channelExtension);
                ...
                if (unitPowerInfo->IdlePowerEnabled) {
                    ...
                    if (storStatus == STOR_STATUS_SUCCESS) {
                        ...
                        // F1 State
                        if (reportF1State) {
                            channelExtension->PoFxDevice->Components[0].FStates[1].Version = STOR_POFX_COMPONENT_IDLE_STATE_VERSION_V1;
                            channelExtension->PoFxDevice->Components[0].FStates[1].Size = STOR_POFX_COMPONENT_IDLE_STATE_SIZE;
                            channelExtension->PoFxDevice->Components[0].FStates[1].TransitionLatency = (ULONGLONG)1000 * 10 * PortGetPoF1Latency(channelExtension);
                            channelExtension->PoFxDevice->Components[0].FStates[1].ResidencyRequirement = (ULONGLONG)1000 * 10 * PortGetPoF1Residency(channelExtension);
                            channelExtension->PoFxDevice->Components[0].FStates[1].NominalPower = STOR_POFX_UNKNOWN_POWER;

                        }

                        //
                        // If using ACPI to invoke DEVSLP, set flag to indicate that the dump version 
                        // of miniport is not capable of bringing device to active if device was put into
                        // either the idle or power off state when idle.
                        //
                        if (channelExtension->StateFlags.SupportsAcpiDSM == 1) {
                    ...
                    if (storStatus != STOR_STATUS_SUCCESS) {
                        ...
                        storStatus = StorPortPoFxSetComponentLatency(AdapterExtension, 
                                                                     (PSTOR_ADDRESS)storAddrBtl8, 
                                                                     0, 
                                                                     (ULONGLONG)1000 * 10 * PortGetPoF1Latency(channelExtension));
                        NT_ASSERT(storStatus == STOR_STATUS_SUCCESS);

                        storStatus = StorPortPoFxSetComponentResidency(AdapterExtension, 
                                                                       (PSTOR_ADDRESS)storAddrBtl8, 
                                                                       0, 
                                                                       (ULONGLONG)1000 * 10 * PortGetPoF1Residency(channelExtension));
                        NT_ASSERT(storStatus == STOR_STATUS_SUCCESS);
        ...
        case ScsiUnitPoFxPowerSetFState: {
            ...
            if ( IsPortValid(adapterExtension, storAddrBtl8->Path) && PortPoFxEnabled(adapterExtension->PortExtension[storAddrBtl8->Path]) ) {
                ...
                if (fStateContext->FState == 0) {
                    // F0 State, come out of DEVSLP
                    PortPhyOnline(adapterExtension->PortExtension[storAddrBtl8->Path]);
                } else if (fStateContext->FState == 1) {
                    // F1 State, come into DEVSLP
                    if (IsPortSoftwareManagedDEVSLPEnabled(adapterExtension->PortExtension[storAddrBtl8->Path])) {
                        PortPhySleep(adapterExtension->PortExtension[storAddrBtl8->Path]);
                    } else {
                        NT_ASSERT(FALSE);
                        status = ScsiUnitControlUnsuccessful;
                    }
