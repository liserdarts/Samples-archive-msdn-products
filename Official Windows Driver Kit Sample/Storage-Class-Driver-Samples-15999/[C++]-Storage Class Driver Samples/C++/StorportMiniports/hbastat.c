...
*/
{
    if ( (ChannelExtension->StartState.ChannelNextStartState >= WaitOnDET) &&
         (ChannelExtension->StartState.ChannelNextStartState <= WaitOnBSYDRQ) ) {
        // Starting Port Process is already in progress.
        return;
    }
...
*/
{
  ...
  //1.1 First, is the channel initialized.  Would turning start on blow up the machine?
    if( !IsPortStartCapable(ChannelExtension) ) {
        ...
        RunNextPort(ChannelExtension, ChannelExtension->StartState.AtDIRQL);
        return FALSE;
    }
...
*/
{
    ...
    if (IsAhciDEVSLPEnabled(ChannelExtension) || IsAcpiDEVSLPSupported(ChannelExtension) || IsPortD3ColdEnabled(ChannelExtension)) {
        // if DEVSLP or D3 Cold is enable, a device was there; Add addtional wait time to make sure device is able to get ready after a possible power on.
