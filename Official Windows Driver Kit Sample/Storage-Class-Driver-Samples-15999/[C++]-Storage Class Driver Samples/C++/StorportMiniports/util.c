    ...
    )
{
  ...
  //1 if link speed was limited, restore the supported value.
    ...
    if (sctl.SPD != ChannelExtension->AdapterExtension->CAP.ISS) {
    ...
    if (isSenseSrb) {
      ...
      //2.2.2 If this was marked as needing return data, fill in the return Task File.
        if( IsReturnResults(srbExtension->Flags) ) {
            ...
            if (srbToRelease->Cdb[0] == SCSIOP_ATA_PASSTHROUGH16) {
                // for ATA PASS THROUGH 16 command, return Descriptor Format Sense Data, including ATA Status Return info.
                if (srbToRelease->SenseInfoBufferLength >= sizeof(DESCRIPTOR_SENSE_DATA)) {
                    PDESCRIPTOR_SENSE_DATA descriptorSenseData = (PDESCRIPTOR_SENSE_DATA)srbToRelease->SenseInfoBuffer;
                    ...
                    AhciZeroMemory(srbToRelease->SenseInfoBuffer, srbToRelease->SenseInfoBufferLength);
                    ...
                    ataStatus->Extend = Is48BitCommand(srbExtension->Flags);
