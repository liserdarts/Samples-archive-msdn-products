...
NcFindFilesBySidTranslateBuffers (
    ...
    _Out_ PFILE_NAME_INFORMATION OutputUserBuffer,
...
NcUsnTranslateBuffers (
    ...
    _Out_ PUSN_RECORD OutputUserBuffer,
    _In_ ULONG InputBufferLength,
    _In_ ULONG OutputBufferLength,
    _Out_ PULONG InputBufferConsumed,
    _Out_ PULONG OutputBufferWritten
    ...
    *OutputBufferWritten = 0;
    ...
    try {
        while( SourceEntry ) {
            ...
            if (SourceEntry->FileNameLength + *InputBufferConsumed + FIELD_OFFSET( FILE_NAME_INFORMATION, FileName ) > InputBufferLength) {
            ...
            RealOverlap = NcComparePath( &NameString,
            ...
            if (ReturnName != NULL) {
                ...
                if (EntryLength + *OutputBufferWritten > OutputBufferLength) {
            ...
            SourceEntry = Add2Ptr( SourceEntry, EntryLength);
            *InputBufferConsumed += EntryLength;
...
--*/
{
    ...
    UserOverlap = NcComparePath( &FileInfo->Name,
                                 &InstanceContext->Mapping.UserMapping,
                                 NULL,
                                 IgnoreCase,
                                 TRUE );

    RealOverlap = NcComparePath( &FileInfo->Name,
                                 &InstanceContext->Mapping.RealMapping,
                                 NULL,
                                 IgnoreCase,
                                 TRUE );
...
#pragma warning(suppress: 6014)
        SourceBuffer = ExAllocatePoolWithTag( PagedPool,
                                              SizeActuallyReturned + FindBySidCtx->BufferSize - FindBySidCtx->CurrentEntry,
                                              NC_TAG );
...
--*/
{
    ...
    while (SourceEntry) {
        ...
        Overlap = NcComparePath( &NameString,
                                 &InstanceContext->Mapping.RealMapping,
                                 &Remainder,
                                 IgnoreCase,
                                 FALSE );
    ...
    *OutputBufferWritten = 0;
    ...
    try {
        while( SourceEntry ) {
            ...
            //  bytes containing the record length, check against that.
            //

            if (*InputBufferConsumed + sizeof(USN_RECORD) > InputBufferLength ||
                *InputBufferConsumed + SourceEntry->RecordLength > InputBufferLength ||
                *InputBufferConsumed + SourceEntry->FileNameOffset + SourceEntry->FileNameLength > InputBufferLength ||
            ...
            *InputBufferConsumed += SourceEntry->RecordLength;
            SourceEntry = Add2Ptr( SourceEntry, SourceEntry->RecordLength );
            *OutputBufferWritten += EntryLength;
            DestEntry = Add2Ptr( DestEntry, EntryLength );
