    ...
    *OutputBufferWritten = 0;
    ...
    try {
        ...
        while (SourceEntry) {
            ...
            if (SourceEntry->FileNameLength + *InputBufferConsumed + FIELD_OFFSET(FILE_NOTIFY_INFORMATION, FileName) > InputBufferLength) {
            ...
            *InputBufferConsumed += EntryLength;
            ...
            if (*InputBufferConsumed >= InputBufferLength || EntryLength == 0) {
...
--*/
{
    ...
    UserOverlap = NcComparePath( &FileNameInformation->Name,
                                 &InstanceContext->Mapping.UserMapping,
                                 NULL,
                                 IgnoreCase,
                                 TRUE );

    RealOverlap = NcComparePath( &FileNameInformation->Name,
                                 &InstanceContext->Mapping.RealMapping,
                                 NULL,
                                 IgnoreCase,
                                 TRUE );
...
--*/
{
    ...
    if (SizeActuallyReturned > FIELD_OFFSET( FILE_NOTIFY_INFORMATION, FileName )) {
