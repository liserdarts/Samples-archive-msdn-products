  ...
  --*/
{
    ...
    while (!nextIrp && !IsListEmpty (&DeviceExtension->ReadQueue)){
        ...
        if (oldCancelRoutine) {
            ...
            //
            NT_ASSERT(oldCancelRoutine == MouseClassCancel);
        }
        else {
            //
            // This IRP was just cancelled and the cancel routine was (or will
    ...
    if (NULL == Globals.GrandMaster) {
        ...
        do {
            ...
            if(!NT_SUCCESS(status)) {
                MouPrint((
                    1,
                    "MOUCLASS-MouCreateClassObject: Could not create device object named %ws",
...
--*/
{
    ...
    if (data->WaitWakeEnabled) {
        ...
        itemData = (PMOUSE_WORK_ITEM_DATA)
                ExAllocatePool (NonPagedPoolNx, sizeof (MOUSE_WORK_ITEM_DATA));
...
--*/
{
    ...
    MouPrint((2,"Mouclass:WW IRP %p complete. Status=%!STATUS!", Irp, IoStatus->Status));
    ...
    // Release the remove lock acquired in MouseClassPower
    IoReleaseRemoveLock (&data->RemoveLock, Irp);
...
--*/
{
    ...
    case IRP_MN_WAIT_WAKE:
        ...
        MouPrint((2,"Mouclass: Received WW Irp %p", Irp));
        ...
        // These tags will be useful in MouseClassWaitWakeComplete to 
