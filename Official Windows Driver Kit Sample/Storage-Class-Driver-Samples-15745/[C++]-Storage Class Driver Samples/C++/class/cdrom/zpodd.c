...
--*/
{
    ...
    d3ColdInterface->SetD3ColdSupport(d3ColdInterface->Context, TRUE);
    ...
    status = DeviceSendEnableIdlePowerIoctl(DeviceExtension, TRUE, TRUE);

    if (!NT_SUCCESS(status))
    {
        d3ColdInterface->SetD3ColdSupport(d3ColdInterface->Context, FALSE);
    ...
    if (inHomePosition == FALSE)
    {
        ...
        DeviceMarkActive(DeviceExtension, TRUE);
    }
    else
    {
        //
...
Cleanup:
    ...
    if (!NT_SUCCESS(status))
    {
        ...
        tempStatus = DeviceSendEnableIdlePowerIoctl(DeviceExtension, FALSE, FALSE);

        if (NT_SUCCESS(tempStatus))
        {
            //
            // Take an active reference; this reference will never be released so that the ODD
            // will never power off.
            //

            DeviceSendIoctlAsynchronously(DeviceExtension, IOCTL_STORAGE_POWER_ACTIVE, DeviceExtension->LowerPdo);
        }
...
--*/
{
    ...
    idlePower.D3IdleTimeout = DELAY_TIME_TO_ENTER_ZERO_POWER_IN_MS;
...
--*/
{
    ...
    if (zpoddInfo->IsActive != IsActive)
    {
        if (IsActive == FALSE)
        {
            // cache get configuration response.
            ...
            if (zpoddInfo->GetConfigurationBuffer == NULL)
            {
                status = DeviceGetConfigurationWithAlloc(DeviceExtension->Device,
                                                         &zpoddInfo->GetConfigurationBuffer,
                                                         &zpoddInfo->GetConfigurationBufferSize,
                                                         FeatureProfileList,
                                                         SCSI_GET_CONFIGURATION_REQUEST_TYPE_ALL);

                UNREFERENCED_PARAMETER(status); // Avoid PREFAST warning.
            }
        }
        ...
        {
            DeviceSendIoctlAsynchronously(DeviceExtension, IOCTL_STORAGE_POWER_IDLE, DeviceExtension->LowerPdo);
        }

        zpoddInfo->IsActive = IsActive;
    }
}
