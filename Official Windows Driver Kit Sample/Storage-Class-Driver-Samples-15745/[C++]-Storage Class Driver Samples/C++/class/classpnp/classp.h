...
#define CLASS_DEFAULT_IDLE_TIMEOUT_IN_SECONDS       (60 * 60)   // 60 minutes
...
extern BOOLEAN MediaPollingNoKeepAwake;
...
IO_WORKITEM_ROUTINE CleanupTransferPacketToWorkingSetSizeWorker;
VOID CleanupTransferPacketToWorkingSetSize(PDEVICE_OBJECT Fdo, BOOLEAN LimitNumPktToDelete);
...
#define NT_SEVERITY(_Status)          (((ULONG)(_Status)) >> 30)

/*
 *  ClasspIsWorseStatus: returns TRUE if Status1 is more severe than Status2, FALSE otherwise
 */
__inline
BOOLEAN
ClasspIsWorseStatus(
    _In_ NTSTATUS Status1,
    _In_ NTSTATUS Status2
    )
{
    //
    // Always propagate STATUS_VERIFY_REQUIRED since it indicates that the media may have changed
    //
    if (Status1 == STATUS_VERIFY_REQUIRED) {
        return TRUE;
    } else if (Status2 == STATUS_VERIFY_REQUIRED) {
        return FALSE;
    }

    //
    // Otherwise base decision on severity of error
    //
    return (NT_SEVERITY(Status1) > NT_SEVERITY(Status2));
}
