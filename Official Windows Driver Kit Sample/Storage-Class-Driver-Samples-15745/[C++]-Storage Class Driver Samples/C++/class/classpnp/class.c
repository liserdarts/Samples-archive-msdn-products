...
volatile ULONG TokenOperationListIdentifier = -1;
...
//
ULONG DiskIdleTimeoutInMS = CLASS_DEFAULT_IDLE_TIMEOUT_IN_SECONDS * 1000;

#ifndef IOCTL_TAPE_BASE
...
NTSTATUS ClassPnpStartDevice(IN PDEVICE_OBJECT DeviceObject)
{
        ...
        if (isFdo){
            ...
            if ((DeviceObject->DeviceType == FILE_DEVICE_DISK) &&
                ...
                if ( NT_SUCCESS(status) &&
                    ...
                    //
                    // field "IoTimeoutValue" is only supported for Storport.
                    //
    ...
    if (NT_SUCCESS(status)){
        ...
        if(commonExtension->IsFdo) {
            ...
            //
            if (fdoExtension->FunctionSupportInfo->IdlePower.IdlePowerEnabled == FALSE &&
                ...
                if (commonExtension->DriverExtension->InitData.ClassStartIo) {
                    ...
                    if (commonExtension->IsFdo){
                        ...
                        if (ClasspIsIdleRequestSupported(fdoData, Irp)) {
                            ...
                            //
                            ClassAcquireRemoveLock(DeviceObject, (PIRP)&uniqueAddr);
                            ...
                            ClassReleaseRemoveLock(DeviceObject, (PIRP)&uniqueAddr);
                        }
                    }
                    else {
                        /*
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (match == FALSE) {
            if (commonExtension->CurrentState != IRP_MN_START_DEVICE)
            {
                ...
                {
                    NT_ASSERT(!"Read capacity recursion detected!");
                } else {
            ...
            switch (senseKey) {
                ...
                case SCSI_SENSE_HARDWARE_ERROR: {
                    ...
                    *Status = STATUS_IO_DEVICE_ERROR;
                    ...
                    if (addlSenseCode == SCSI_ADSENSE_LUN_COMMUNICATION &&
                    ...
                    } else if ((addlSenseCode == SCSI_ADSENSE_LOGICAL_UNIT_ERROR && addlSenseCodeQual == SCSI_SENSEQ_TIMEOUT_ON_LOGICAL_UNIT) ||
                        ...
                        // So log a more descriptive error.
                        //
                        ...
                        //
                        // Avoid a second error message (IO_ERR_CONTROLLER_ERROR) being logged.
                        //
                        logError = FALSE;
                    }
                    ...
                    switch (addlSenseCode) {
                        ...
                        case SCSI_ADSENSE_INVALID_FIELD_PARAMETER_LIST: {
                            ...
                            switch (addlSenseCodeQual) {
                                ...
                                //
                                // TODO: Alignment violation uses InvalidReleaseOfPR as a placeholder
                    ...
                    switch (addlSenseCode) {
                        ...
                        case SCSI_ADSENSE_OPERATING_CONDITIONS_CHANGED: {
                            ...
                            if (addlSenseCodeQual == SCSI_SENSEQ_INQUIRY_DATA_CHANGED) {
                                ...
                                //
                                // Next time anyone trying to query the FunctionSupportInfo, we will requery
                                // the device.
                                //
                                InterlockedIncrement(&fdoExtension->FunctionSupportInfo->ChangeRequestCount);
                            }
...
__ClassInterpretSenseInfo_ProcessingInvalidSenseBuffer:
     ...
     */
    {
         ...
         */
        if (retry && RetryCount == 1 && logRetryableError && IS_SCSIOP_READWRITE(cdbOpcode)) {
    ...
    )
{
    ...
    if (senseBufferMdl){
        ...
        if (pkt){
            ...
            KeInitializeEvent(&event, SynchronizationEvent, FALSE);
            SetupModeSenseTransferPacket(pkt, &event, ModeSenseBuffer, (UCHAR)Length, PageMode, 0, &pseudoIrp, MODE_SENSE_CURRENT_VALUES);
            SubmitTransferPacket(pkt);
            (VOID)KeWaitForSingleObject(&event, Executive, KernelMode, FALSE, NULL);
    ...
    )
{
    ...
    while (retryList != NULL) {
        ...
        if (NO_REMOVE == InterlockedCompareExchange(&commonExtension->IsRemoved, REMOVE_PENDING, REMOVE_PENDING)) {
    ...
    if ((NT_SUCCESS(status)) &&
        ...
        if ((descriptorType == BLOCK_DEVICE_TOKEN_LIMITS_DESCRIPTOR_TYPE_WINDOWS) &&
            ...
            TracePrint((TRACE_LEVEL_INFORMATION,
                        ...
                        DeviceObject,
                        ((PUCHAR)fdoExtension->DeviceDescriptor) + fdoExtension->DeviceDescriptor->VendorIdOffset,
                        ((PUCHAR)fdoExtension->DeviceDescriptor) + fdoExtension->DeviceDescriptor->ProductIdOffset,
                        ((PUCHAR)fdoExtension->DeviceDescriptor) + fdoExtension->DeviceDescriptor->ProductRevisionOffset,
...

{
    ...
    //
    // In case we aren't able to get the activityId from the IRP, we'll use the IRP
    // address as the activityId as fallback plan.
    //
    RtlCopyMemory(&activityId, (PGUID)Irp, sizeof(GUID));
...

{
    ...
    InterlockedCompareExchange(&TokenOperationListIdentifier, -1, MaxTokenOperationListIdentifier);
    listIdentifier = InterlockedIncrement(&TokenOperationListIdentifier);
...

{
    ...
    NT_ASSERT(0 == estimatedRetryInterval);
...

{
    ...
    //
    if (MAX_TARGET_DURATION <= (KeQueryInterruptTime() - OffloadWriteContext->OperationStartTime)) {
...

{
    ...
    // TODO: jaivira: Uncomment after storage partners and Hyper-V have updated their firmware and code
...
--*/
{
    ...
    if (generationCount == changeRequestCount) {
        return STATUS_SUCCESS;
    }
...
--*/
{
    ...
    status = ClasspRefreshFunctionSupportInfo(FdoExtension);
