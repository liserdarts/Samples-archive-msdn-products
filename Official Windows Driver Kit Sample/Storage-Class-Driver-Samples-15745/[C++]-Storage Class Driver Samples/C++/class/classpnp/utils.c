...
Arguments:
    ...
    MaxLbaCount - This is the max number of LBAs that can be sent in a single
        unmap command.
    UnmapGranularity - This is the unmap granularity in blocks.  This is used
        to split up the LBAs to achieve an optimal unmap command.
...
--*/
{
    ...
    while ( (DataSetRange->LengthInBytes > 0) &&
        ...
        // single descriptor.  If it is too large, reduce it to the largest
        // granularity-algined value it can be.
        //
        if (sectorCount > MAXULONG) {
            sectorCount = (MAXULONG / UnmapGranularity) * UnmapGranularity;
        }

        //
        // The max LBA count is the max number of LBAs that can be unmapped with
        // a single UNMAP command.  Make sure we don't exceed this value and keep
        // the sectorCount granularity-aligned if we have to cap it.
        //
        if ((*CurrentLbaCount + sectorCount) >= MaxLbaCount) {
            sectorCount = ((MaxLbaCount / UnmapGranularity) * UnmapGranularity) - *CurrentLbaCount;
        }
...
--*/
{
    ...
    //
    // The granularity should never be zero.
    //
    NT_ASSERT(UnmapGranularity != 0);
    ...
    for (i = 0; i < DataSetRangesCount; i++) {
        ...
        if (lbaCount > 0) {
            //
            // Calculate the max LBAs we can put in a descriptor that are also granularity-aligned.
            //
            ULONG maxLBAsPerDescr = (MAXULONG / UnmapGranularity) * UnmapGranularity;

            neededBlockDescrCount += (ULONG)((lbaCount - 1) / maxLBAsPerDescr + 1);
        }
    }
    ...
    while (!allDataSetRangeFullyConverted) {
        ...
        if ( (blockDescrIndex == maxBlockDescrCount) ||         // Buffer full or block descriptor count reached
            ...
            TracePrint((TRACE_LEVEL_INFORMATION,
                        TRACE_FLAG_IOCTL,
                        "UNMAP/TRIM command issued. NT Status: %!STATUS!.\n",
                        status
                        ));
            //
...
*/
{
    ...
    CLASS_VPD_B0_DATA           blockLimitsData;
    ULONG                       generationCount;
    BOOLEAN                     blockLimitsDataIsOutdated;
        ...
        case NotSupported: {
            ...
            if (ClasspSupportsUnmap(fdoExtension->FunctionSupportInfo))
            {
                ...
                else
                {
                    ...
                    TracePrint((TRACE_LEVEL_INFORMATION,
                                TRACE_FLAG_GENERAL,
                                "ClasspDeviceTrimProcess: Optimal Unmap Granularity not provided, defaulted to 1.\n"));
                }
                ...
                else
                {
                    ...
                    TracePrint((TRACE_LEVEL_INFORMATION,
                                TRACE_FLAG_GENERAL,
                                "ClasspDeviceTrimProcess: Unmap Granularity Alignment not provided, defaulted to 0.\n"));
                }
                    ...
                    for (i = 0; i < dataSetRangesCount; i++)
                    {
                            ...
                            ((ULONGLONG)dataSetRanges[i].StartingOffset + dataSetRanges[i].LengthInBytes > (ULONGLONG)fdoExtension->CommonExtension.PartitionLength.QuadPart))
                        {
                            TracePrint((TRACE_LEVEL_ERROR,
                                        TRACE_FLAG_IOCTL,
                                        "ClasspDeviceTrimProcess: Invalid dataset range.  StartingOffset = %I64x, LengthInBytes = %I64x\n",
                ...
                {
                    {
                        ...
                        if (fdoExtension->FunctionSupportInfo->ValidInquiryPages.LBProvisioning &&
                            ...
                            TracePrint((TRACE_LEVEL_ERROR,
                                        TRACE_FLAG_IOCTL,
                                        "ClasspDeviceTrimProcess: Device %p does not support file level TRIM.\n",
                                        DeviceObject));
                ...
                TracePrint((TRACE_LEVEL_ERROR,
                            TRACE_FLAG_IOCTL,
                            "ClasspDeviceTrimProcess: Device does not support UNMAP.\n"));
...
--*/
{
    ...
    TracePrint((TRACE_LEVEL_INFORMATION,
                TRACE_FLAG_IOCTL,
                "GetLBAStatus command issued. NT Status: %!STATUS!.\n",
                status
                ));
...
--*/
{
    ...
    *BlockLimitsDataMayHaveChanged = FALSE;
    ...
    bytesPerSlab = blocksPerSlab * fdoExtension->DiskGeometry.BytesPerSector;

    //
    // Calculate the maximum number of slabs that could be returned by Get LBA Status.
    //
    maxBufferSize = MIN(MAXULONG, fdoExtension->AdapterDescriptor->MaximumTransferLength);
    ...
    {
        startingLBA = (((StartingOffset - alignmentInBytes) / bytesPerSlab) + 1) * blocksPerSlab + alignmentInBlocks;
        startingOffsetDelta = (startingLBA * fdoExtension->DiskGeometry.BytesPerSector) - StartingOffset;
    }
    else
    {
        startingLBA = ((StartingOffset - alignmentInBytes) / bytesPerSlab) * blocksPerSlab + alignmentInBlocks;
    ...
    //
    // Validate the length requested.  The following are error conditions:
    // The caller asked for no slabs.
    // The caller asked for more slabs than maximum that could be returned by
    // Get LBA Status.
    //
    if (requestedSlabs == 0 || requestedSlabs > maxSlabs)
    {
        status = STATUS_INVALID_PARAMETER;
        goto Exit;
    }
    ...
    //
    // Make sure the output buffer is large enough to handle the case where
    // Get LBA Status returns a descriptor for each slab requested.
    // The required output buffer length is equal to:
    //  1. The size of DEVICE_MANAGE_DATA_SET_ATTRIBUTES_OUTPUT; plus
    //  2. The size of DEVICE_DATA_SET_LB_PROVISIONING_STATE; plus
    //  3. The size of a ULONG array large enough to hold a bit for each slab requested.
    //     (The first element is already allocated in DEVICE_DATA_SET_LB_PROVISIONING_STATE.)
    //
    ...
    //
    if (*DsmOutputLength < requiredOutputLength)
    {
        status = STATUS_BUFFER_TOO_SMALL;

        //
        // If the output buffer is big enough, write the requred buffer
        // length to the first ULONG bytes of the output buffer.
        //
        if (*DsmOutputLength >= sizeof(ULONG))
        {
    ...
    //
    lbaStatusSize = (ULONG)(sizeof(LBA_STATUS_LIST_HEADER) + (requestedSlabs * sizeof(LBA_STATUS_DESCRIPTOR)));
#if defined (_ARM_)
    //
    // ARM has specific alignment requirements, although this will not have a functional impact on x86 or amd64
    // based platforms. We are taking the conservative approach here.
    ...
    //
    // Send down GetLBAStatus for the dataset range.
    //
    ...
    //
    // Set default values for the output buffer.  We will only change these if
    // we succeed at everything below.
    //
    ...
    if (NT_SUCCESS(status))
    {
        ...
        if (descrSize < descrSizeOverhead)
        {
            TracePrint((TRACE_LEVEL_ERROR,
                        TRACE_FLAG_IOCTL,
                        "ClasspDeviceGetLBAStatus: not enough bytes returned\n"));
        ...
        if (descrCount == 0)
        {
            TracePrint((TRACE_LEVEL_ERROR,
                        TRACE_FLAG_IOCTL,
                        "ClasspDeviceGetLBAStatus: Device returned no LBA Status Descriptors.\n"));
        ...
        //
        // The returned DEVICE_DATA_SET_LB_PROVISIONING_STATE is at the end
        // of the DSM output structure.  Zero it out before using it.
        //
        lbpState = (PDEVICE_DATA_SET_LB_PROVISIONING_STATE)((PUCHAR)DsmOutput + sizeof(DEVICE_MANAGE_DATA_SET_ATTRIBUTES_OUTPUT));
        RtlZeroMemory(lbpState, *DsmOutputLength - sizeof(DEVICE_MANAGE_DATA_SET_ATTRIBUTES_OUTPUT));
        ...
        //
        for (descrIndex = 0; descrIndex < descrCount && slabTotal < requestedSlabs && !stopProcessing; descrIndex++)
        {
            ...
            if (returnedStartingLBA != expectedStartingLBA)
            {
                ...
                TracePrint((TRACE_LEVEL_ERROR,
                            TRACE_FLAG_IOCTL,
                            "ClasspDeviceGetLBAStatus: Device returned starting LBA = %I64x when %I64x was expected.\n",
                            returnedStartingLBA,
                            startingLBA));
            ...
            else if (lbaCount > 0 && lbaCount % blocksPerSlab != 0)
            {
                ...
                TracePrint((TRACE_LEVEL_WARNING,
                        TRACE_FLAG_IOCTL,
                        "ClasspDeviceGetLBAStatus: Device returned an LBA count (%u) that is not a multiple of the slab size (%u)\n",
                        lbaCount,
                        blocksPerSlab));
            ...
            {
                //
                // If the LBA count is 0, don't process any more descriptors and return
                // whatever we currently have.
                //
                TracePrint((TRACE_LEVEL_WARNING,
                        TRACE_FLAG_IOCTL,
                        "ClasspDeviceGetLBAStatus: Device returned a zero LBA count\n"));
            ...
            {
                slabCount = lbaCount / blocksPerSlab;

                for(slab = 0; slab < slabCount && slabTotal < requestedSlabs; slab++, bitCount++, slabTotal++)
                {
                    ULONG bitMapIndex = bitCount / bitMapGranularityInBits;
                    ULONG bitPos = bitCount % bitMapGranularityInBits;
        ...
        if (bitCount == 0)
        {
            ...
            TracePrint((TRACE_LEVEL_ERROR,
                        TRACE_FLAG_IOCTL,
                        "ClasspDeviceGetLBAStatus: The slab allocation bitmap has zero length.\n"));

            NT_ASSERT(bitCount != 0);
            status = STATUS_UNSUCCESSFUL;
            goto Exit;
        }

        //
        // Finish filling out the output buffer.
        // Note that there is already one element of the bitmap array allocated
        // in the DEVICE_DATA_SET_LB_PROVISIONING_STATE structure itself.
        //
        lbpState->SlabSizeInBytes = bytesPerSlab;
        lbpState->SlabAllocationBitMapBitCount = bitCount;
        lbpState->SlabAllocationBitMapLength = ((bitCount - 1) / bitMapGranularityInBits) + 1;
        ...
        lbpState->Version = sizeof(DEVICE_DATA_SET_LB_PROVISIONING_STATE);

        DsmOutput->OutputBlockOffset = sizeof(DEVICE_MANAGE_DATA_SET_ATTRIBUTES_OUTPUT);
        DsmOutput->OutputBlockLength = lbpState->Size;
        *DsmOutputLength = DsmOutput->Size + DsmOutput->OutputBlockLength;
    } else {
        //
        // The Get LBA Status command failed, so we just return the DSM output
        // header.
        //
...
--*/
{
    ...
    REVERSE_BYTES_SHORT(&(cdb->LOGSENSE.ParameterPointer), LogPage);
...
--*/
{
    ...
    //
    // This functionality is only supported for thinly provisioned devices.
    //
    if (fdoExtension->FunctionSupportInfo->ReadCapacity16Data.LBProvisioningEnabled == FALSE)
    {
        TracePrint((TRACE_LEVEL_ERROR,
            TRACE_FLAG_GENERAL,
            "ClassGetLBProvisioningResources: DO (%p), Function not supported, device is not thinly provisioned.\n",
            DeviceObject));
...
--*/
{
    ...
    if (NT_SUCCESS(ClasspLogSystemEventWithDeviceNumber(DeviceObject, IO_WARNING_DISK_CAPACITY_CHANGED))) {
...
--*/
{
    ...
    if (commonExtension->IsFdo)
    {
        ...
        if (workItem)
        {
            ...
            // system event log.
            //
            IoQueueWorkItem(workItem, ClassLogCapacityChangedEvent, DelayedWorkQueue, workItem);
        }
        else
        {
            TracePrint((TRACE_LEVEL_ERROR,
...
--*/
{
    ...
    switch (ioLogMessageContextHeader->ErrorCode) {
        ...
        case IO_WARNING_IO_OPERATION_RETRIED: {
            //
            // We need to allocate enough space for 2 insertion strings:
            // The first is a ULONGLONG representing the LBA which means a max of 20 digits,
            // plus one for the NULL character.
            // The second is a ULONG representing the disk number in decimal, which means
            // a max of 10 digits, plus one for the NULL character.
            //
            stringsBufferLength = (21 + 11) * sizeof(WCHAR);
    ...
    // insertion strings.
    //
    logEntrySize = sizeof(IO_ERROR_LOG_PACKET) + sizeof(ULONG) + senseBufferSize + stringsBufferLength;
    ...
    if (errorLogEntry) {
        ...
        errorLogEntry->DumpData[0] = (ULONG)ioLogMessageContextHeader->OpCode;
        ...
        stringSize = logEntrySize - (sizeof(IO_ERROR_LOG_PACKET) + errorLogEntry->DumpDataSize);
        ...
        switch (ioLogMessageContextHeader->ErrorCode) {
            ...
            case IO_WARNING_IO_OPERATION_RETRIED: {
                ...
                //
                // The first is a ULONGLONG representing the LBA plus the NULL character.
                // The second is a ULONG representing the disk number plus the NULL character.
                //
                status = RtlStringCbPrintfW(stringIndex, stringSize, L"%I64x", ioLogMessageContext->Lba.QuadPart);
                if (NT_SUCCESS(status)) {
                    errorLogEntry->NumberOfStrings++;
...
--*/
{
    ...
    TracePrint((TRACE_LEVEL_INFORMATION,
                TRACE_FLAG_GENERAL,
                "ClasspQueueLogIOEventWithContextWorker: DO (%p), Queueing IO retried event log message work item.\n",
                DeviceObject));
                ...
                case SRB_FUNCTION_EXECUTE_SCSI:
                    ...
                    switch (srbExData->Type) {
                        ...
                        case SrbExDataTypeScsiCdb16:
                            Srb->ScsiStatus = ((PSRBEX_DATA_SCSI_CDB16)Srb)->ScsiStatus;
                            Srb->CdbLength = ((PSRBEX_DATA_SCSI_CDB16)Srb)->CdbLength;
                            Srb->SenseInfoBufferLength = ((PSRBEX_DATA_SCSI_CDB16)Srb)->SenseInfoBufferLength;
                            Srb->SenseInfoBuffer = ((PSRBEX_DATA_SCSI_CDB16)Srb)->SenseInfoBuffer;
                            RtlCopyMemory(Srb->Cdb, ((PSRBEX_DATA_SCSI_CDB16)Srb)->Cdb, sizeof(Srb->Cdb));
                            foundEntry = TRUE;
                            break;

                        case SrbExDataTypeScsiCdb32:
                            Srb->ScsiStatus = ((PSRBEX_DATA_SCSI_CDB32)Srb)->ScsiStatus;
                            Srb->CdbLength = ((PSRBEX_DATA_SCSI_CDB32)Srb)->CdbLength;
                            Srb->SenseInfoBufferLength = ((PSRBEX_DATA_SCSI_CDB32)Srb)->SenseInfoBufferLength;
                            Srb->SenseInfoBuffer = ((PSRBEX_DATA_SCSI_CDB32)Srb)->SenseInfoBuffer;

                            // Copy only the first 16 bytes
                            RtlCopyMemory(Srb->Cdb, ((PSRBEX_DATA_SCSI_CDB32)Srb)->Cdb, sizeof(Srb->Cdb));
                            foundEntry = TRUE;
                            break;

                        case SrbExDataTypeScsiCdbVar:
                            Srb->ScsiStatus = ((PSRBEX_DATA_SCSI_CDB_VAR)Srb)->ScsiStatus;
                            Srb->CdbLength = (UCHAR)((PSRBEX_DATA_SCSI_CDB_VAR)Srb)->CdbLength;
                            Srb->SenseInfoBufferLength = ((PSRBEX_DATA_SCSI_CDB_VAR)Srb)->SenseInfoBufferLength;
                            Srb->SenseInfoBuffer = ((PSRBEX_DATA_SCSI_CDB_VAR)Srb)->SenseInfoBuffer;

                            // Copy only the first 16 bytes
                            RtlCopyMemory(Srb->Cdb, ((PSRBEX_DATA_SCSI_CDB_VAR)Srb)->Cdb, sizeof(Srb->Cdb));
                            foundEntry = TRUE;
                            break;
...

{
    ...
    copyOffloadDescr = (PDEVICE_COPY_OFFLOAD_DESCRIPTOR)Irp->AssociatedIrp.SystemBuffer;
...

{
    ...
    maxLbaPerDescr = fdoExtension->FunctionSupportInfo->BlockLimitsData.MaximumTransferLength;
    optimalLbaPerDescrGranularity = fdoExtension->FunctionSupportInfo->BlockLimitsData.OptimalTransferLengthGranularity;
    optimalLbaPerDescr = fdoExtension->FunctionSupportInfo->BlockLimitsData.OptimalTransferLength;
