...
ClasspPowerUpCompletionFailure:
        ...
        case PowerUpDeviceUnlocked: {
            ...
            //
            // Enable tick timer at end of D0 processing if it was enabled
            //
    ...
    )
{
    ...
    switch(PowerContext->PowerChangeState.PowerDown3) {
        ...
        case PowerDownDeviceUnlocked3: {
            ...
            //
            // Disable tick timer at end of D3 processing if running.
            //
            if (fdoExtension->PrivateFdoData && (fdoExtension->PrivateFdoData->TickTimerEnabled)) {
...
_IRQL_requires_same_
NTSTATUS
ClasspQueryD3ColdInterface(
    _In_ PDEVICE_OBJECT DeviceObject
    )
/*++
Description:

    This function sends a query for the D3Cold support interface to the top of
    the device stack.

    If the interface was successfully obtained, this function will fill in the
    D3ColdInterface member of the FDO's device extension.
...
Return Value:
    ...
    STATUS_SUCCESS if the D3Cold support interface exists. Otherwise, it will be
    some other failure status.
...
--*/
{
    ...
    RtlZeroMemory(&(fdoExtension->FunctionSupportInfo->IdlePower.D3ColdInterface),
                  sizeof(fdoExtension->FunctionSupportInfo->IdlePower.D3ColdInterface));
    ...
    //
    // Send the query interface request to the top of the stack so that we can
    // be certain it hits the ACPI filter driver.
    //
    ...
    if (irp) {
        ...
        irpStack->Parameters.QueryInterface.Size = sizeof(fdoExtension->FunctionSupportInfo->IdlePower.D3ColdInterface);
        irpStack->Parameters.QueryInterface.Version = D3COLD_SUPPORT_INTERFACE_VERSION;
        irpStack->Parameters.QueryInterface.Interface = (PINTERFACE)&(fdoExtension->FunctionSupportInfo->IdlePower.D3ColdInterface);
...
Description:
    ...
    This function is used to determine if the platform supports D3Cold for the
    given device.

    This function will also update the D3ColdSupported field in the FDO's
    device extension to reflect the platform's D3Cold support.

Arguments:

    DeviceObject - The class FDO.
    D3ColdSupported - Pointer to a boolean that will indicate if the platform
        supports D3Cold or not for the given device.

Return Value:

    Some NTSTATUS code indicating the status of querying the platform for
    D3Cold support.  If the code indicates failure, this function assumes the
    platform does not support D3Cold.

--*/
{
    NTSTATUS status = STATUS_UNSUCCESSFUL;
    PFUNCTIONAL_DEVICE_EXTENSION fdoExtension = (PFUNCTIONAL_DEVICE_EXTENSION)DeviceObject->DeviceExtension;
    PD3COLD_SUPPORT_INTERFACE d3ColdInterface = &fdoExtension->FunctionSupportInfo->IdlePower.D3ColdInterface;
    
    *D3ColdSupported = FALSE;        

    //
    // Query for the D3Cold support interface if we haven't already.
    //
    if (d3ColdInterface->Size == 0) {
        status = ClasspQueryD3ColdInterface(DeviceObject);
    } else if (d3ColdInterface->Size == sizeof(D3COLD_SUPPORT_INTERFACE)) {
        status = STATUS_SUCCESS;
    }

    //
    // If the interface exists and the GetD3ColdCapability function exists,
    // call it to see if the platform supports D3Cold for this device.
    //
    if (NT_SUCCESS(status) &&
        d3ColdInterface->GetD3ColdCapability != NULL) {

        status = d3ColdInterface->GetD3ColdCapability(d3ColdInterface->Context, D3ColdSupported);
    ...
    fdoExtension->FunctionSupportInfo->IdlePower.D3ColdSupported = *D3ColdSupported;
...
Description:
    ...
    This function is used to set D3Cold support for the given device.

    This function will also update the D3ColdEnabled field in the FDO's
    device extension to reflect the setting of D3Cold support.

    Note that the platform must support D3Cold in order for enabling
    D3Cold to work completely.

Arguments:

    DeviceObject - The class FDO.
    Enable - TRUE if the caller wants to enable D3Cold support for the device,
        FALSE otherwise.

Return Value:

    Some NTSTATUS code indicating the status of the enable.  Because the D3Cold
    interface's SetD3ColdSupport() function does not return a status code, this
    function assumes the call to it was successful.
...
--*/
{
    ...
    if (NT_SUCCESS(status) && 
        d3ColdInterface->SetD3ColdSupport) {
        d3ColdInterface->SetD3ColdSupport(d3ColdInterface->Context, Enable);
        fdoExtension->FunctionSupportInfo->IdlePower.D3ColdEnabled = Enable;
        status = STATUS_SUCCESS;
    }
...
--*/
{
    ...
    idlePower.D3ColdSupported = fdoExtension->FunctionSupportInfo->IdlePower.D3ColdEnabled;
    ...
    if (IsEqualGUID(SettingGuid, &GUID_DISK_IDLE_TIMEOUT)) {
        ...
        if ((ValueLength == sizeof(ULONG)) && (Value != NULL)) {
            if (*((PULONG)Value) == PowerMonitorOff) {
                MediaPollingNoKeepAwake = TRUE;
            } else {
                MediaPollingNoKeepAwake = FALSE;
            }
        }
...
--*/
{
    ...
    //
    // If the device is not hot-pluggable and the user has not explicitly
    // disabled D3Cold, determine if the platform supports D3Cold for this
    // device. If so, enable D3Cold for this device.
    //
    if (!d3ColdDisabledByUser &&
        !fdoExtension->PrivateFdoData->HotplugInfo.DeviceHotplug) {

        status = ClasspDetermineD3ColdSupport(DeviceObject, &d3ColdSupported);
        if (NT_SUCCESS(status) && d3ColdSupported) {
            ClasspSetD3ColdSupport(DeviceObject, TRUE);
        } else {
            d3ColdSupported = FALSE;
        }
    }
    ...
    if (!NT_SUCCESS(status)) {
        //
        // If we failed to register for runtime idle power management, then we
        // need to disable D3Cold for this device.
        //
        if (d3ColdSupported) {
            ClasspSetD3ColdSupport(DeviceObject, FALSE);
        }
    }
