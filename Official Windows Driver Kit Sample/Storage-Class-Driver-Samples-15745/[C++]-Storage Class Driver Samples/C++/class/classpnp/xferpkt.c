...
NTSTATUS TransferPktComplete(IN PDEVICE_OBJECT NullFdo, IN PIRP Irp, IN PVOID Context)
{
    ...
    if (packetDone){
        ...
        /*
         *  Propagate informational (success) errors up the stack.
         *  Precedence is given however, to more severe errors. i.e.
         *  The original IRP should get a failing error code
         *  if any one of the packets failed.
         */
        while (TRUE) {

            NTSTATUS currentStatus = pkt->OriginalIrp->IoStatus.Status;

            //
            // Best case scenario: we don't need to update the original IRP's status
            //
            if (!ClasspIsWorseStatus(Irp->IoStatus.Status, currentStatus)) {
                break;
            }

            //
            // This thread needs to update the original IRP's status. If it is successful while contending
            // with another thread that is also trying to update the IRP's status, break out. If it loses
            // out to the other thread, the next iteration will determine again if it needs to update the
            // original IRP's status.
            //
            if (currentStatus == InterlockedCompareExchange(&pkt->OriginalIrp->IoStatus.Status, Irp->IoStatus.Status, currentStatus)) {
                break;
            }
        }
