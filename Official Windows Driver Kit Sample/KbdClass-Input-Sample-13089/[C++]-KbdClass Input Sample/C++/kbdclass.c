...

{
    ...
    NT_ASSERT(numPorts <= MAXIMUM_PORTS_SERVICED);
  ...
  * --*/
{
    ...
    if (Globals.ConnectOneClassToOnePort) {
        ...
        NT_ASSERT(NULL == Globals.GrandMaster);
...

{
    ...
    case IOCTL_KEYBOARD_SET_IME_STATUS:
        ...
        if (loopit) {
            ...
            ExAcquireFastMutex (&Globals.Mutex);
            callAll = ExAllocatePool (NonPagedPoolNx,
...
--*/
{
    ...
    NT_ASSERT(Globals.GrandMaster->Self == Device);
    ...
    if (CallAll->Current < CallAll->Len) {
        ...
        //
        NT_ASSERT(Globals.GrandMaster == Device->DeviceExtension);
...

{
    ...
    irpSp = IoGetCurrentIrpStackLocation(Irp);
    NT_ASSERT(IRP_MJ_CREATE == irpSp->MajorFunction);
    ...
    if (deviceExtension->Self == deviceExtension->TrueClassDevice) {
        ...
        if (SeSinglePrivilegeCheck(priv, Irp->RequestorMode)) {
            ...
            NT_ASSERT(!IS_TRUSTED_FILE_FOR_READ (irpSp->FileObject));
...

{
    ...
    NT_ASSERT(IRP_MJ_CLOSE == irpSp->MajorFunction);

    if (deviceExtension->Self == deviceExtension->TrueClassDevice) {

        KeAcquireSpinLock(&deviceExtension->SpinLock, &oldIrql);
        if (IS_TRUSTED_FILE_FOR_READ (irpSp->FileObject)) {
            NT_ASSERT(0 < deviceExtension->TrustedSubsystemCount);
            deviceExtension->TrustedSubsystemCount--;
    ...
    if ((Globals.GrandMaster == deviceExtension) && (0 == --Globals.Opens)) {
        ...
        for (i = 0; i < Globals.NumAssocClass; i++) {
            ...
            if (enabled) {
                ...
                NT_ASSERT(NULL == port->Port->TargetNotifyHandle);
            }
  ...
  --*/
{
    ...
    NT_ASSERT(DeviceExtension->InputCount != 0);
  ...
  --*/
{
    ...
    if (DeviceExtension->InputCount == 0) {
        ...
        //
        NT_ASSERT(IsListEmpty (&DeviceExtension->ReadQueue));
  ...
  --*/
{
    ...
    while (!nextIrp && !IsListEmpty (&DeviceExtension->ReadQueue)){
        ...
        if (oldCancelRoutine) {
            ...
            //
                NT_ASSERT(oldCancelRoutine == KeyboardClassCancel);
        }
        else {
            //
                // This IRP was just cancelled and the cancel routine was (or will
...

{
    ...
    for (entry = Globals.LegacyDeviceList.Flink;
        ...
        NT_ASSERT(data->PnP == FALSE);

        if (Globals.GrandMaster) {
            port = &Globals.AssocClassList[data->UnitId];
            NT_ASSERT(port->Port == data);
    ...
    if (Globals.AssocClassList) {
        ...
        for (i = 0; i < Globals.NumAssocClass; i++) {
            NT_ASSERT(Globals.AssocClassList[i].Free == TRUE);
            NT_ASSERT(Globals.AssocClassList[i].Enabled == FALSE);
            NT_ASSERT(Globals.AssocClassList[i].File == NULL);
        }
#endif
...
KbdCreateClassObject(
    _In_  PDRIVER_OBJECT      DriverObject,
    _In_  PDEVICE_EXTENSION   TmpDeviceExtension,
    _Outptr_result_nullonfailure_
    _At_(*ClassDeviceObject, __drv_allocatesMem(Mem))
         PDEVICE_OBJECT     * ClassDeviceObject,
    _Outptr_result_maybenull_ PWSTR   * FullDeviceName,
    _In_  BOOLEAN             Legacy
    )
...

{
    ...
    *ClassDeviceObject = NULL;
    ...
    if (NULL == Globals.GrandMaster) {
        ...
        // overflow.
        //
        if (Globals.BaseClassName.Length > MAXULONG - (sizeof(L"\\Device\\") + sizeof(name) + sizeof(UNICODE_NULL))) {
            status = STATUS_UNSUCCESSFUL;
        ...
        do {
            if (Globals.ConnectOneClassToOnePort && Legacy) {
                status = RtlUnicodeStringPrintf(&fullClassName, L"\\Device\\%wZLegacy%d",&Globals.BaseClassName,nameIndex++);                
            }
            else {
                status = RtlUnicodeStringPrintf(&fullClassName, L"\\Device\\%wZ%d",&Globals.BaseClassName,nameIndex++);
            }               
                
            if(!NT_SUCCESS(status)) {
                KbdPrint((
                    1,
                    "KBDCLASS-KbdCreateClassObject: Could not create device object named %ws",
...

{
    ...
    if ((Port->TrueClassDevice == Port->Self) && (Port->PnP)) {
        ...
        if (EnableFlag) {
            ...
            // grand master.
            //
            NT_ASSERT(IRP_MJ_CREATE == stack->MajorFunction);
            ...
            if (IRP_MJ_CLOSE != stack->MajorFunction) {
                ...
                // underneath us.
                //
                NT_ASSERT(IRP_MJ_PNP == stack->MajorFunction);
                NT_ASSERT((IRP_MN_REMOVE_DEVICE == stack->MinorFunction) ||
        ...
        // Master.
        //
        NT_ASSERT(Port->TrueClassDevice == Globals.GrandMaster->Self);
        ...
        if (EnableFlag) {
            ...
            status = IoGetDeviceProperty (
                             ...
                             buffer,
                             &bufferLength);
            NT_ASSERT(STATUS_BUFFER_TOO_SMALL == status);
            ...
            if (NT_SUCCESS (status)) {
                ...
                //
                NT_ASSERT(NULL == Port->TargetNotifyHandle);
    ...
    if (Data->Started) {
        ...
        if (Globals.GrandMaster) {
            if (0 < Globals.Opens) {
                ...
                //
                // NT_ASSERT(NULL == Data->notifyHandle);
                //
                ...
                NT_ASSERT(!Globals.AssocClassList[Data->UnitId].Enabled);
                ExReleaseFastMutex (&Globals.Mutex);
            }
        }
        else {
            ExReleaseFastMutex (&Globals.Mutex);
            NT_ASSERT(Data->TrueClassDevice == Data->Self);
            NT_ASSERT(Globals.ConnectOneClassToOnePort);
...
--*/
{
    ...
    if (NT_SUCCESS (Irp->IoStatus.Status)) {
        ...
        if (WAITWAKE_SUPPORTED (data)) {
            ...
            NT_ASSERT(2 == data->WmiLibInfo.GuidCount);
            ...
            NT_ASSERT(1 == data->WmiLibInfo.GuidCount);
        }
        ...
        if (Globals.GrandMaster) {
            ...
            ExReleaseFastMutex (&Globals.Mutex);
            NT_ASSERT(data->Self == data->TrueClassDevice);
...
--*/
{
    ...
    case IRP_MN_START_DEVICE:
        ...
        //
        item = ExAllocatePool(NonPagedPoolNx, sizeof(DEVICE_START_WORKER));
        if (item) {
            ...
            if (Globals.GrandMaster) {
                if (0 < Globals.Opens) {
                    ...
                    if (enabled) {
                        ...
                        NT_ASSERT(NULL == data->TargetNotifyHandle);
                    }
                } else {
                    NT_ASSERT(!Globals.AssocClassList[data->UnitId].Enabled);
                    ExReleaseFastMutex (&Globals.Mutex);
    ...
    case IRP_MN_REMOVE_DEVICE:
        ...
        // Before sending the remove irp down, wait for everyone holding
        // the remove lock to release it
        ...
        ExAcquireFastMutex (&Globals.Mutex);
        if (Globals.GrandMaster) {
            NT_ASSERT(Globals.GrandMaster->Self == data->TrueClassDevice);
            //
            // We must remove ourself from the Assoc List
            //

            if (1 < Globals.NumAssocClass) {
                NT_ASSERT(Globals.AssocClassList[data->UnitId].Port == data);
                ...
                NT_ASSERT(1 == Globals.NumAssocClass);
...
--*/
{
    ...
    NT_ASSERT(Globals.GrandMaster->Self == Port->TrueClassDevice);
...
--*/
{
    ...
    if (data->WaitWakeEnabled) {
        ...
        itemData = (PKEYBOARD_WORK_ITEM_DATA)
                ExAllocatePool (NonPagedPoolNx, sizeof (KEYBOARD_WORK_ITEM_DATA));
...
--*/
{
    ...
    NT_ASSERT(MinorFunction == IRP_MN_WAIT_WAKE);
    //
    // PowerState.SystemState is undefined when the WW irp has been completed
    //
    // NT_ASSERT(PowerState.SystemState == PowerSystemWorking);

    if (InterlockedExchangePointer(&data->ExtraWaitWakeIrp, NULL)) {
        NT_ASSERT(IoStatus->Status == STATUS_INVALID_DEVICE_STATE);
    } else {
        InterlockedExchangePointer(&data->WaitWakeIrp, NULL);
    }

    // Get the tag(Irp) to be used for ReleaseRemoveLock
    Irp = InterlockedExchangePointer(&data->ExtraWWIrpTag, NULL);
    if (!Irp) {
        Irp = InterlockedExchangePointer(&data->WWIrpTag, NULL);
    }

    KbdPrint((2,"KbdClass:WW Irp %p complete. Status=%!STATUS!", Irp, IoStatus->Status));
    ...
    // Release the remove lock acquired in KeyboardClassPower
    IoReleaseRemoveLock (&data->RemoveLock, Irp);
}
...
--*/
{
    ...
    BOOLEAN         releaseRemoveLock = TRUE;
    ...
    if (data == Globals.GrandMaster) {
        ...
        //
        NT_ASSERT(data != Globals.GrandMaster);
    ...
    case IRP_MN_QUERY_POWER:
        NT_ASSERT(SystemPowerState == powerType);
    ...
    case IRP_MN_WAIT_WAKE:
        ...
        KbdPrint((2,"KbdClass: Received WW Irp %p", Irp));
        // RemoveLock is acquired near the beginning of this function.
        // For WaitWake Irp, just keep track of whether this is an extra
        // WaitWake or 1st WaitWake with WWIrpTag and ExtraWWIrpTag tags.
        // These tags will be useful in KeyboardClassWaitWakeComplete to 
        // call the corresponding ReleaseRemoveLock
        releaseRemoveLock = FALSE;
        ...
        if (InterlockedCompareExchangePointer(&data->WaitWakeIrp,
            ...
            */
            ...
            InterlockedExchangePointer(&data->ExtraWWIrpTag, Irp); // Store the tag
            status = STATUS_INVALID_DEVICE_STATE;
        }
        else {            
            InterlockedExchangePointer(&data->WWIrpTag, Irp); // Store the tag
            status = STATUS_SUCCESS;
        }
        break;
    ...
    // The RemLock acquired for WaitWake will be released
    // in MouseClassWaitWakeComplete
    if (releaseRemoveLock) {
        IoReleaseRemoveLock (&data->RemoveLock, Irp);
    }
    ...
    )
{
    ...
    if (ARGUMENT_PRESENT(S_Irp)) {
        ...
        //
        NT_ASSERT(data->SystemState == PowerSystemWorking);
    ...
    )
{
    ...
    case IRP_MN_SET_POWER:
        switch (powerType) {
        case DevicePowerState:
            NT_ASSERT(powerState.DeviceState < data->DeviceState);
            //
            // Powering up
            //
            PoSetPowerState (data->Self, powerType, powerState);
        ...
        case SystemPowerState:
            NT_ASSERT(powerState.SystemState < data->SystemState);
            //
            // Powering up
            //
    ...
    default:
        NT_ASSERT(0xBADBAD == stack->MinorFunction);
        break;
    }
