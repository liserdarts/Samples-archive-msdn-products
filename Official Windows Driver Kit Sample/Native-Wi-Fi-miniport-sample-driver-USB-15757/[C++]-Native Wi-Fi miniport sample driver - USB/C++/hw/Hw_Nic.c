...
HwSetRFState(
    _In_  PNIC                pNic,
    _In_  RT_RF_POWER_STATE   NewRFState
    )
{
...
HwDoze(
      _In_  PNIC     Nic,
      _In_  ULONG    ulDozeTime)       // in micro-second
{
...
HwSetKey(
        _In_  PNIC Nic,
        _In_  PNICKEY pNicKey,
        _In_  UCHAR keyIndex,
        _In_  BOOLEAN perStaKey,
...
Hw11SetPowerMgmtMode(
                    _In_  PNIC Nic,
                    _In_  PDOT11_POWER_MGMT_MODE mode
                    )
{
    NdisAcquireSpinLock(&Nic->ManagementInfo.PowerMgmtLock);
